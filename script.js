...
const inventoryBackpackSlot = document.getElementById('inventoryBackpackSlot');
...
const chestRecoveryPanel = document.getElementById('chestRecoveryPanel');
const chestRecoveryHint = document.getElementById('chestRecoveryHint');
const chestRecoveryActions = document.getElementById('chestRecoveryActions');
...
const upgradeBackpackSlot = document.getElementById('upgradeBackpackSlot');
...
const inventoryWorldCameraPosition = new THREE.Vector3();
const inventoryWorldCharacterPosition = new THREE.Vector3();
const inventoryWorldPointer = new THREE.Vector2();
const inventoryWorldScale = new THREE.Vector3();
const inventoryTextureLoader = new THREE.TextureLoader();
...
const inventoryNoticeState = {
  message: '',
  expiresAt: 0,
};

const inventoryWorldState = {
  enabled: false,
  built: false,
  group: new THREE.Group(),
  boardGroup: null,
  lightGroup: null,
  inventorySlotCapacity: 0,
  slotRecords: [],
  slotRecordMap: new Map(),
  raycastables: [],
  headSlotRecord: null,
  backpackSlotRecord: null,
  snapshot: null,
  iconTextures: new Map(),
  badgeTextures: new Map(),
  characterYawOffset: 0,
  characterZoom: 2.78,
  characterDragActive: false,
  characterDragPointerId: null,
  characterDragLastX: 0,
  characterDragSensitivity: 0.012,
};
...
const inventoryState = {
      ...
      itemSlots: Array.from({ length: 12 }, () => null),
...
const BACKPACK_MAX_LEVEL = 5;
const INVENTORY_DROP_PICKUP_RADIUS = 0.85;
const INVENTORY_DROP_UNLOCK_RADIUS = 1.28;
const INVENTORY_DROP_GRAVITY = 8.8;
const INVENTORY_DROP_PICKUP_DELAY = 0.42;
...
const HOME_CHEST_RECOVERY_ITEM_KINDS = ['backpack', 'flashlight', 'sword', 'pickaxe'];

const itemKindEmptyHints = {
  inventory: 'Arraste itens entre slots',
  head: 'Slot de cabeca',
  backpack: 'Slot da mochila',
...
const backpackDefinitions = [
  { level: 1, name: 'Mochila Rustica', extraSlots: 3, cost: null, primary: 0x6b4226, secondary: 0x5a3520, pocket: 0x4a2e15, badge: 0x8a7a5a, glow: 0x8b7355 },
  { level: 2, name: 'Mochila do Explorador', extraSlots: 6, cost: { ferrita: 4, cobre: 1 }, primary: 0x3a5a32, secondary: 0x2a3a22, pocket: 0x4a4a3a, badge: 0x777766, glow: 0x6a6a55 },
  { level: 3, name: 'Mochila do Minerador', extraSlots: 9, cost: { ferrita: 6, cobre: 3 }, primary: 0x2a4466, secondary: 0x1e3350, pocket: 0x333d4d, badge: 0x44aaff, glow: 0x2266aa },
  { level: 4, name: 'Mochila Arcana', extraSlots: 12, cost: { ferrita: 8, cobre: 5, ametista: 2 }, primary: 0x3a1a4a, secondary: 0x2a1038, pocket: 0x2a1535, badge: 0xdd88ff, glow: 0xaa44ff },
  { level: 5, name: 'Mochila do Abismo', extraSlots: 15, cost: { ferrita: 10, cobre: 6, ametista: 3, safira: 1 }, primary: 0x1a1215, secondary: 0x1a1015, pocket: 0x15100d, badge: 0xffd700, glow: 0xff8800 },
];

function createHtmlBackpackRoundedBox(width, height, depth, radius, segments = 3) {
...
function attachHtmlBackpackParticles(group, particleSystems, color, count, radius) {
  ...
  for (let index = 0; index < count; index++) {
    ...
    positions[index * 3 + 1] = Math.random() * 2.5 + 0.2;
  ...
  group.add(points);
  particleSystems.push({ points, positions, velocities, radius });
  return points;
}

function createLiteralHtmlBackpackModel(level = 1) {
  const safeLevel = THREE.MathUtils.clamp(Math.floor(Number(level)) || 1, 1, BACKPACK_MAX_LEVEL);
  ...
  const addMesh = (geometry, material, px, py, pz, rx = 0, ry = 0, rz = 0) => {
  ...
  if (safeLevel === 1) {
    const bodyMat = new THREE.MeshStandardMaterial({ color: 0x6b4226, roughness: 0.85, metalness: 0.05 });
    const strapMat = new THREE.MeshStandardMaterial({ color: 0x4a2e15, roughness: 0.75, metalness: 0.05 });
    const buckleMat = new THREE.MeshStandardMaterial({ color: 0x8a7a5a, roughness: 0.4, metalness: 0.7 });

    addMesh(createHtmlBackpackRoundedBox(1.1, 1.3, 0.6, 0.08), bodyMat, 0, 0.9, 0);
    addMesh(
      createHtmlBackpackRoundedBox(1.14, 0.4, 0.06, 0.05),
      new THREE.MeshStandardMaterial({ color: 0x5a3520, roughness: 0.8, metalness: 0.05 }),
      0,
      1.55,
      -0.22,
      -0.25,
    ...
    addMesh(new THREE.TorusGeometry(0.06, 0.015, 8, 12), buckleMat, 0, 1.35, -0.33);

    for (const side of [-1, 1]) {
      addMesh(new THREE.BoxGeometry(0.1, 1.4, 0.04), strapMat, side * 0.35, 0.8, 0.32);
      addMesh(new THREE.TorusGeometry(0.1, 0.025, 8, 12, Math.PI), strapMat, side * 0.35, 1.5, 0.22, 0, Math.PI / 2, 0);
    }

    const stitchMat = new THREE.MeshBasicMaterial({ color: 0x8b7355, toneMapped: false });
    const stitchGeo = new THREE.BoxGeometry(0.95, 0.015, 0.005);
    for (const y of [0.45, 1.35]) {
      const stitch = new THREE.Mesh(stitchGeo, stitchMat);
      stitch.position.set(0, y, -0.31);
      group.add(stitch);
    }
  } else if (safeLevel === 2) {
    const bodyMat = new THREE.MeshStandardMaterial({ color: 0x3a5a32, roughness: 0.75, metalness: 0.1 });
    const accentMat = new THREE.MeshStandardMaterial({ color: 0x2a3a22, roughness: 0.7, metalness: 0.1 });
    const strapMat = new THREE.MeshStandardMaterial({ color: 0x4a4a3a, roughness: 0.65, metalness: 0.15 });
    const metalMat = new THREE.MeshStandardMaterial({ color: 0x777766, roughness: 0.3, metalness: 0.8 });

    addMesh(createHtmlBackpackRoundedBox(1.2, 1.45, 0.7, 0.08), bodyMat, 0, 0.95, 0);
    addMesh(createHtmlBackpackRoundedBox(0.8, 0.5, 0.15, 0.05), accentMat, 0, 0.6, -0.38);

    for (const side of [-1, 1]) {
      addMesh(createHtmlBackpackRoundedBox(0.15, 0.45, 0.4, 0.04), accentMat, side * 0.65, 0.6, 0);
    }

    addMesh(createHtmlBackpackRoundedBox(1.24, 0.45, 0.07, 0.05), accentMat, 0, 1.68, -0.28, -0.2, 0, 0);

    for (const x of [-0.25, 0.25]) {
      addMesh(new THREE.BoxGeometry(0.08, 0.12, 0.03), metalMat, x, 1.42, -0.37);
    }

    for (const side of [-1, 1]) {
      addMesh(new THREE.BoxGeometry(0.13, 1.5, 0.05), strapMat, side * 0.38, 0.85, 0.38);
      addMesh(createHtmlBackpackRoundedBox(0.16, 0.4, 0.07, 0.03), strapMat, side * 0.38, 0.95, 0.4);
    }

    addMesh(
      createHtmlBackpackRoundedBox(1.22, 0.15, 0.72, 0.03),
      new THREE.MeshStandardMaterial({ color: 0x2a2a22, roughness: 0.6, metalness: 0.2 }),
      0,
      0.23,
      0
    );
    addMesh(new THREE.CylinderGeometry(0.12, 0.12, 1.0, 16), new THREE.MeshStandardMaterial({ color: 0x6a6a55, roughness: 0.8 }), 0, 1.85, 0.1, 0, 0, Math.PI / 2);
  } else if (safeLevel === 3) {
    const bodyMat = new THREE.MeshStandardMaterial({ color: 0x2a4466, roughness: 0.55, metalness: 0.25 });
    const panelMat = new THREE.MeshStandardMaterial({ color: 0x1e3350, roughness: 0.5, metalness: 0.3 });
    const metalMat = new THREE.MeshStandardMaterial({ color: 0x99aabb, roughness: 0.2, metalness: 0.9 });
    ...
    addMesh(createHtmlBackpackRoundedBox(1.3, 1.55, 0.75, 0.06, 4), bodyMat, 0, 1.0, 0);

    [
      { pos: [0, 1.78, 0], size: [1.34, 0.04, 0.79] },
      { pos: [0, 0.22, 0], size: [1.34, 0.04, 0.79] },
      { pos: [-0.67, 1.0, 0], size: [0.04, 1.6, 0.79] },
      { pos: [0.67, 1.0, 0], size: [0.04, 1.6, 0.79] },
    ].forEach((frame) => {
      addMesh(new THREE.BoxGeometry(...frame.size), metalMat, ...frame.pos);
    });

    for (let index = 0; index < 2; index++) {
      const x = index === 0 ? -0.28 : 0.28;
      addMesh(createHtmlBackpackRoundedBox(0.5, 0.45, 0.12, 0.03), panelMat, x, 0.6, -0.42);
      const light = addMesh(new THREE.SphereGeometry(0.025, 12, 12), glowMat, x, 0.82, -0.48);
      glowMeshes.push(light);
    }

    addMesh(createHtmlBackpackRoundedBox(1.1, 0.3, 0.12, 0.04), panelMat, 0, 1.5, -0.4);

    for (const side of [-1, 1]) {
      addMesh(new THREE.CylinderGeometry(0.1, 0.1, 0.5, 12), metalMat, side * 0.72, 0.7, 0);
      addMesh(
        new THREE.CylinderGeometry(0.11, 0.11, 0.04, 12),
        new THREE.MeshStandardMaterial({ color: 0x44aaff, metalness: 0.8, roughness: 0.2 }),
        side * 0.72,
        0.96,
        0
      );
      addMesh(new THREE.BoxGeometry(0.14, 1.55, 0.05), new THREE.MeshStandardMaterial({ color: 0x333d4d, roughness: 0.6, metalness: 0.2 }), side * 0.4, 0.9, 0.4);
      for (const py of [0.5, 0.9, 1.3]) {
        addMesh(
          createHtmlBackpackRoundedBox(0.18, 0.15, 0.08, 0.02),
          new THREE.MeshStandardMaterial({ color: 0x2a4466, roughness: 0.5, metalness: 0.3 }),
          side * 0.4,
          py,
          0.43
        );
    ...
    addMesh(new THREE.CylinderGeometry(0.025, 0.025, 0.9, 8), new THREE.MeshStandardMaterial({ color: 0x6a5030, roughness: 0.8 }), 0.82, 1.1, 0.1, 0, 0, 0.15);
    addMesh(new THREE.BoxGeometry(0.3, 0.06, 0.06), metalMat, 0.82, 1.55, 0.1);
  } else if (safeLevel === 4) {
    const bodyMat = new THREE.MeshStandardMaterial({ color: 0x3a1a4a, roughness: 0.45, metalness: 0.35 });
    const trimMat = new THREE.MeshStandardMaterial({ color: 0xc8a030, roughness: 0.2, metalness: 0.9 });
    ...
    const runeMat = new THREE.MeshStandardMaterial({ color: 0xdd88ff, emissive: 0xaa44dd, emissiveIntensity: 1.0 });

    addMesh(createHtmlBackpackRoundedBox(1.3, 1.6, 0.75, 0.07, 5), bodyMat, 0, 1.05, 0);

    [
      { pos: [0, 1.86, 0], size: [1.36, 0.05, 0.81] },
      { pos: [0, 0.24, 0], size: [1.36, 0.05, 0.81] },
      { pos: [-0.68, 1.05, 0], size: [0.05, 1.67, 0.81] },
      { pos: [0.68, 1.05, 0], size: [0.05, 1.67, 0.81] },
      { pos: [0, 1.05, -0.4], size: [1.36, 1.67, 0.03] },
    ].forEach((trim) => {
      if (trim.size[2] > 0.5) return;
      addMesh(new THREE.BoxGeometry(...trim.size), trimMat, ...trim.pos);
    });

    addMesh(
      createHtmlBackpackRoundedBox(1.34, 0.5, 0.08, 0.05),
      new THREE.MeshStandardMaterial({ color: 0x2a1038, roughness: 0.4, metalness: 0.35 }),
      0,
      1.82,
      -0.28,
      -0.18,
    ...
    const crystal = addMesh(new THREE.OctahedronGeometry(0.15, 0), crystalMat, 0, 1.05, -0.42, 0, Math.PI / 4, 0);
    glowMeshes.push(crystal);

    const crystalLight = new THREE.PointLight(0xaa44ff, 1.5, 3);
    crystalLight.position.copy(crystal.position);
    group.add(crystalLight);

    const runeCircleGeo = new THREE.TorusGeometry(0.2, 0.012, 8, 32);
    for (let index = 0; index < 3; index++) {
      const angle = (index / 3) * Math.PI * 2;
      const rune = addMesh(runeCircleGeo, runeMat, Math.cos(angle) * 0.35, 0.7 + Math.sin(angle) * 0.3, -0.4, 0, 0.1, 0);
      glowMeshes.push(rune);
    }

    const smallCrystalGeo = new THREE.OctahedronGeometry(0.06, 0);
    [[-0.55, 1.7, -0.35], [0.55, 1.7, -0.35], [-0.55, 0.4, -0.35], [0.55, 0.4, -0.35]].forEach((position) => {
      const smallCrystal = addMesh(smallCrystalGeo, crystalMat, ...position, 0, Math.PI / 4, 0);
      glowMeshes.push(smallCrystal);
    });

    for (const side of [-1, 1]) {
      addMesh(
        createHtmlBackpackRoundedBox(0.06, 0.8, 0.5, 0.02),
        new THREE.MeshStandardMaterial({ color: 0x2a1038, roughness: 0.5, metalness: 0.3 }),
        side * 0.7,
        0.8,
        0
      );
      const sideRune = addMesh(new THREE.TorusGeometry(0.1, 0.01, 6, 20), runeMat, side * 0.74, 0.8, 0, 0, Math.PI / 2, 0);
      glowMeshes.push(sideRune);
      addMesh(new THREE.BoxGeometry(0.12, 1.6, 0.04), new THREE.MeshStandardMaterial({ color: 0x2a1535, roughness: 0.5, metalness: 0.3 }), side * 0.4, 0.9, 0.4);
      for (const py of [0.4, 1.4]) {
        addMesh(new THREE.TorusGeometry(0.04, 0.012, 6, 12), trimMat, side * 0.4, py, 0.43);
      }
    }

    attachHtmlBackpackParticles(group, particleSystems, 0xaa44ff, 80, 1.5);
  } else {
    const bodyMat = new THREE.MeshStandardMaterial({ color: 0x1a1215, roughness: 0.35, metalness: 0.5 });
    const goldMat = new THREE.MeshStandardMaterial({ color: 0xffd700, roughness: 0.15, metalness: 0.95, emissive: 0x443300, emissiveIntensity: 0.3 });
    const crystalMat = new THREE.MeshStandardMaterial({
      color: 0xffaa00,
      roughness: 0.05,
      metalness: 0.2,
    ...
    const runeGoldMat = new THREE.MeshStandardMaterial({ color: 0xffd700, emissive: 0xff8800, emissiveIntensity: 1.5 });
    ...
    addMesh(createHtmlBackpackRoundedBox(1.4, 1.7, 0.8, 0.06, 5), bodyMat, 0, 1.1, 0);
    addMesh(new THREE.BoxGeometry(1.46, 0.06, 0.86), goldMat, 0, 1.96, 0);
    addMesh(new THREE.BoxGeometry(1.46, 0.06, 0.86), goldMat, 0, 0.24, 0);
    addMesh(new THREE.BoxGeometry(0.06, 1.78, 0.86), goldMat, -0.73, 1.1, 0);
    addMesh(new THREE.BoxGeometry(0.06, 1.78, 0.86), goldMat, 0.73, 1.1, 0);

    for (const dx of [-0.35, 0.35]) {
      for (const dy of [0.65, 1.45]) {
        addMesh(new THREE.BoxGeometry(0.25, 0.025, 0.02), goldMat, dx, dy, -0.42, 0, 0, Math.PI / 4);
        addMesh(new THREE.BoxGeometry(0.025, 0.25, 0.02), goldMat, dx, dy, -0.42, 0, 0, Math.PI / 4);
      }
    }

    const mainCrystal = addMesh(new THREE.OctahedronGeometry(0.2, 1), crystalMat, 0, 1.1, -0.45, 0, Math.PI / 4, 0);
    glowMeshes.push(mainCrystal);
    addMesh(new THREE.TorusGeometry(0.25, 0.025, 12, 32), goldMat, 0, 1.1, -0.43);
    const innerRing = addMesh(new THREE.TorusGeometry(0.15, 0.015, 8, 24), runeGoldMat, 0, 1.1, -0.44);
    glowMeshes.push(innerRing);

    const mainCrystalLight = new THREE.PointLight(0xff8800, 2.5, 4);
    mainCrystalLight.position.set(0, 1.1, -0.6);
    group.add(mainCrystalLight);

    const gemColors = [0xff2200, 0xff6600, 0xffaa00, 0xff2200, 0xff6600, 0xffaa00];
    ...
    addMesh(new THREE.CylinderGeometry(0.3, 0.35, 0.08, 6), goldMat, 0, 1.98, -0.1);
    for (let index = 0; index < 5; index++) {
      const angle = (index / 5) * Math.PI * 2 - Math.PI / 2;
      addMesh(new THREE.ConeGeometry(0.04, 0.2, 6), goldMat, Math.cos(angle) * 0.22, 2.12, -0.1 + Math.sin(angle) * 0.22);
    }

    const topCrystal = addMesh(new THREE.OctahedronGeometry(0.08, 0), darkCrystalMat, 0, 2.22, -0.1);
    glowMeshes.push(topCrystal);
    const topLight = new THREE.PointLight(0xff4400, 1.0, 2);
    topLight.position.set(0, 2.22, -0.1);
    group.add(topLight);

    addMesh(
      createHtmlBackpackRoundedBox(1.42, 0.35, 0.06, 0.04),
      new THREE.MeshStandardMaterial({ color: 0x1a1015, roughness: 0.35, metalness: 0.5 }),
      0,
      1.9,
      -0.32,
      -0.15,
    ...
    for (const side of [-1, 1]) {
      addMesh(
        createHtmlBackpackRoundedBox(0.08, 1.0, 0.55, 0.02),
        new THREE.MeshStandardMaterial({ color: 0x15100d, roughness: 0.4, metalness: 0.4 }),
        side * 0.77,
        0.9,
        0
      );
      for (let chainIndex = 0; chainIndex < 4; chainIndex++) {
        addMesh(new THREE.TorusGeometry(0.035, 0.01, 6, 10), goldMat, side * 0.82, 0.5 + chainIndex * 0.22, -0.2, chainIndex % 2 === 0 ? 0 : Math.PI / 2, 0, 0);
      }
      const rune = addMesh(new THREE.TorusGeometry(0.12, 0.012, 8, 24), runeGoldMat, side * 0.82, 0.9, 0.05, 0, Math.PI / 2, 0);
      glowMeshes.push(rune);
      addMesh(new THREE.BoxGeometry(0.14, 1.7, 0.04), new THREE.MeshStandardMaterial({ color: 0x1a1215, roughness: 0.4, metalness: 0.4 }), side * 0.42, 0.95, 0.42);
      addMesh(new THREE.BoxGeometry(0.16, 1.72, 0.01), new THREE.MeshStandardMaterial({ color: 0xaa8820, roughness: 0.3, metalness: 0.8 }), side * 0.42, 0.95, 0.45);
      for (const py of [0.35, 0.95, 1.55]) {
        addMesh(new THREE.TorusGeometry(0.05, 0.015, 8, 16), goldMat, side * 0.42, py, 0.46);
      }
    }

    attachHtmlBackpackParticles(group, particleSystems, 0xffaa00, 150, 2.0);
    attachHtmlBackpackParticles(group, particleSystems, 0xff3300, 60, 1.8);
  }

  group.userData.update = (elapsedTime) => {
    for (let index = 0; index < glowMeshes.length; index++) {
      const mesh = glowMeshes[index];
      if (mesh.material?.emissiveIntensity !== undefined) {
        ...
        mesh.material.emissiveIntensity = base + Math.sin(elapsedTime * 2 + index * 0.7) * 0.3;
      }
    ...
    for (const particleSystem of particleSystems) {
      const { points, positions, velocities, radius } = particleSystem;
      for (let index = 0; index < velocities.length; index++) {
        ...
        if (
          positions[index * 3 + 1] > 3.0
          || Math.abs(positions[index * 3]) > radius
          || Math.abs(positions[index * 3 + 2]) > radius
        ) {
          positions[index * 3] = (Math.random() - 0.5) * 1.0;
          positions[index * 3 + 1] = 0.2 + Math.random() * 0.3;
          positions[index * 3 + 2] = (Math.random() - 0.5) * 1.0;
        }
      }
...
function cloneInventoryEntry(item) {
  return item ? { ...item } : null;
}

function setInventoryNotice(message, durationMs = 3200) {
  inventoryNoticeState.message = String(message ?? '').trim();
  inventoryNoticeState.expiresAt = inventoryNoticeState.message ? performance.now() + durationMs : 0;
}

function getActiveInventoryNotice() {
  if (!inventoryNoticeState.message) return '';
  if (performance.now() > inventoryNoticeState.expiresAt) {
    inventoryNoticeState.message = '';
    inventoryNoticeState.expiresAt = 0;
    return '';
  }
  return inventoryNoticeState.message;
}

function getBackpackDefinition(level = 1) {
  const safeLevel = THREE.MathUtils.clamp(Math.floor(Number(level)) || 1, 1, BACKPACK_MAX_LEVEL);
  return backpackDefinitions[safeLevel - 1];
}

function getBackpackExtraSlots(level = 1) {
  return Math.max(0, Math.floor(Number(getBackpackDefinition(level)?.extraSlots ?? 0)) || 0);
}

function getEquippedBackpack(equipment = inventoryState.equipment) {
  return equipment?.backpack?.kind === 'backpack' ? equipment.backpack : null;
}

function getBackpackExtraSlotsFromItem(item) {
  return item?.kind === 'backpack' ? getBackpackExtraSlots(item.level) : 0;
}

function getInventorySlotCapacityForEquipment(equipment = inventoryState.equipment) {
  return BASE_INVENTORY_SLOT_COUNT + getBackpackExtraSlotsFromItem(getEquippedBackpack(equipment));
}

function getInventorySlotCapacity() {
  return getInventorySlotCapacityForEquipment(inventoryState.equipment);
}

function getInventoryGridColumnCount(slotCount = getInventorySlotCapacity()) {
  if (slotCount <= 12) return 3;
  if (slotCount <= 16) return 4;
  return 5;
}

function getUpgradeInventoryColumnCount(slotCount = getInventorySlotCapacity()) {
  return slotCount <= 16 ? 4 : 5;
}
...
function getMaxLevelForItemKind(kind) {
  switch (kind) {
    ...
    case 'backpack': return getUpgradeBalanceMaxLevel('backpack');
    default: return 1;
  }
}
...
function createDefaultItemInventory(levels = {}, fallbackTotals = {}) {
  const hotbarSlots = [
    ...
    ...Array.from({ length: HOTBAR_SLOT_COUNT - 3 }, () => null),
  ];
  const itemSlots = Array.from({ length: BASE_INVENTORY_SLOT_COUNT }, () => null);
  const totals = sanitizeTotals(fallbackTotals);
  let writeIndex = 0;
  itemSlots[writeIndex++] = createInventoryItem('backpack', levels.backpack ?? 1);
  for (const definition of oreDefinitions) {
...
function getAllInventoryItemsSnapshot(state = inventoryState, workbenchItem = workbenchState.item) {
  return [
    ...(state.hotbarSlots ?? []).filter(Boolean),
...
function countItemKindInCollection(items, kind) {
  if (!Array.isArray(items) || !kind) return 0;
  return items.reduce((total, item) => total + (item?.kind === kind ? 1 : 0), 0);
}

function countItemKindEverywhere(kind) {
  if (!kind || !itemKindLabels[kind] || kind === 'ore') return 0;
  let total = 0;
  total += countItemKindInCollection(inventoryState.hotbarSlots, kind);
  total += countItemKindInCollection(inventoryState.itemSlots, kind);
  total += Object.values(inventoryState.equipment ?? {}).reduce((sum, item) => sum + (item?.kind === kind ? 1 : 0), 0);
  total += workbenchState.item?.kind === kind ? 1 : 0;
  total += countItemKindInCollection(chestState.itemSlots, kind);
  total += (world.dropItems ?? []).reduce((sum, drop) => (
    sum + (drop?.dropType === 'item' && drop.item?.kind === kind ? 1 : 0)
  ), 0);
  return total;
}

function hasItemKindAnywhere(kind) {
  return countItemKindEverywhere(kind) > 0;
}

function ensureCoreItemsPresent(inventoryData, fallbackLevels = {}) {
  const requiredKinds = ['crossbow'];
  const existingKinds = new Set(getAllInventoryItemsSnapshot({
    hotbarSlots: inventoryData.hotbarSlots,
  ...
  for (const kind of requiredKinds) {
    ...
    if (kind === 'flashlight' && !inventoryData.equipment.head) {
      ...
      if (emptyIndex !== -1) {
        inventoryData.itemSlots[emptyIndex] = item;
        continue;
      }
      const emptyHotbarIndex = inventoryData.hotbarSlots.findIndex((entry) => !entry);
      if (emptyHotbarIndex !== -1) inventoryData.hotbarSlots[emptyHotbarIndex] = item;
    } else {
      const emptyHotbarIndex = inventoryData.hotbarSlots.findIndex((entry) => !entry);
      if (emptyHotbarIndex !== -1) {
        inventoryData.hotbarSlots[emptyHotbarIndex] = item;
        continue;
      }
      const emptyIndex = inventoryData.itemSlots.findIndex((entry) => !entry);
...
function normalizeItemInventoryData(rawData, fallbackLevels = {}, fallbackTotals = {}) {
  ...
  const hasExplicitHotbar = Array.isArray(rawData?.hotbarSlots);
  const legacyInventoryItems = Array.isArray(rawData?.itemSlots)
    ? rawData.itemSlots.map((item) => sanitizeInventoryItem(item) ?? null)
    : [];
  const migratedHotbarItems = hasExplicitHotbar ? [] : legacyInventoryItems.slice(0, HOTBAR_SLOT_COUNT);
  const migratedInventoryItems = hasExplicitHotbar
    ? []
    : legacyInventoryItems.slice(HOTBAR_SLOT_COUNT, HOTBAR_SLOT_COUNT + BASE_INVENTORY_SLOT_COUNT);
  const equipment = {
    head: sanitizeInventoryItem(rawData?.equipment?.head) ?? null,
    backpack: sanitizeInventoryItem(rawData?.equipment?.backpack) ?? null,
  };
  const desiredCapacity = getInventorySlotCapacityForEquipment(equipment);
  const normalized = {
    itemSlots: hasExplicitHotbar
      ? Array.from({ length: desiredCapacity }, (_, index) => sanitizeInventoryItem(rawData?.itemSlots?.[index]) ?? null)
      : Array.from({ length: desiredCapacity }, (_, index) => migratedInventoryItems[index] ? { ...migratedInventoryItems[index] } : null),
    hotbarSlots: hasExplicitHotbar
      ? Array.from({ length: HOTBAR_SLOT_COUNT }, (_, index) => sanitizeInventoryItem(rawData?.hotbarSlots?.[index]) ?? null)
      : Array.from({ length: HOTBAR_SLOT_COUNT }, (_, index) => migratedHotbarItems[index] ? { ...migratedHotbarItems[index] } : null),
    equipment,
...
function collectOverflowInventoryItems(rawData) {
  if (!rawData?.itemSlots || !Array.isArray(rawData.itemSlots)) {
    return [];
  }
  ...
  const capacity = Array.isArray(rawData.hotbarSlots)
    ? getInventorySlotCapacityForEquipment(normalizedEquipment)
    : BASE_INVENTORY_SLOT_COUNT + HOTBAR_SLOT_COUNT;
  if (rawData.itemSlots.length <= capacity) return [];
  return rawData.itemSlots
    .slice(capacity)
    .map((item) => sanitizeInventoryItem(item))
    .filter(Boolean);
}

function storeItemsInSlotCollection(slotCollection, items = []) {
  const leftovers = [];
  if (!Array.isArray(slotCollection) || !Array.isArray(items)) return leftovers;

  for (const originalItem of items) {
    const item = cloneInventoryEntry(originalItem);
    if (!item) continue;
    if (item.kind === 'ore') {
      let remaining = Math.max(0, Math.floor(Number(item.amount)) || 0);
      for (const slotItem of slotCollection) {
        if (!remaining || slotItem?.kind !== 'ore' || slotItem.oreId !== item.oreId || slotItem.amount >= ORE_STACK_LIMIT) continue;
        const moved = Math.min(ORE_STACK_LIMIT - slotItem.amount, remaining);
      ...
      for (let index = 0; index < slotCollection.length && remaining > 0; index++) {
        ...
        slotCollection[index] = createOreStackItem(item.oreId, moved);
        remaining -= moved;
      }
      if (remaining > 0) leftovers.push({ ...item, amount: remaining });
      continue;
    }
    const emptyIndex = slotCollection.findIndex((entry) => !entry);
    if (emptyIndex !== -1) slotCollection[emptyIndex] = item;
...
function storeOverflowItemsInChest(items = []) {
  if (!Array.isArray(items) || !items.length) return [];
  const leftovers = storeItemsInSlotCollection(chestState.itemSlots, items);
  recalculateResourceTotals();
  return leftovers;
}

function getInventoryEntryUnitCount(item) {
  if (!item) return 0;
  return item.kind === 'ore'
    ? Math.max(0, Math.floor(Number(item.amount)) || 0)
    : 1;
}

function addInventoryItemToInventorySlots(item) {
  const entry = cloneInventoryEntry(item);
  if (!entry) return null;
  const initialCount = getInventoryEntryUnitCount(entry);
  const leftovers = storeItemsInSlotCollection(inventoryState.itemSlots, [entry]);
  const leftover = leftovers[0] ? cloneInventoryEntry(leftovers[0]) : null;
  if (getInventoryEntryUnitCount(leftover) !== initialCount) {
    recalculateResourceTotals();
  }
  return leftover;
}

function canDropItemFromLocation(location) {
  return Boolean(location && ['inventory', 'hotbar', 'equipment', 'workbench'].includes(location.type));
}

function restoreInventoryMutationSnapshot(snapshot) {
  if (!snapshot) return;
  inventoryState.itemSlots = snapshot.itemSlots.map((item) => cloneInventoryEntry(item));
  inventoryState.hotbarSlots = snapshot.hotbarSlots.map((item) => cloneInventoryEntry(item));
  chestState.itemSlots = snapshot.chestSlots.map((item) => cloneInventoryEntry(item));
  inventoryState.equipment = Object.fromEntries(
    Object.entries(snapshot.equipment ?? {}).map(([slot, item]) => [slot, cloneInventoryEntry(item)])
  );
  workbenchState.item = cloneInventoryEntry(snapshot.workbenchItem);
}

function dropInventoryItemsNearPlayer(items = [], {
  notice = false,
  scatterRadius = 0.34,
  pickupDelay = INVENTORY_DROP_PICKUP_DELAY,
} = {}) {
  const droppedItems = (items ?? []).map((item) => cloneInventoryEntry(item)).filter(Boolean);
  if (!droppedItems.length) return 0;
  getPlayerInteractionOrigin(interactionOrigin);
  spawnDroppedInventoryItems(droppedItems, interactionOrigin, { scatterRadius, pickupDelay });
  if (notice) {
    setInventoryNotice(
      droppedItems.length === 1
        ? `${getItemDisplayName(droppedItems[0])} caiu no chao.`
        : `${droppedItems.length} item(ns) excedente(s) cairam no chao.`
    );
  }
...
function applyNormalizedItemInventory(normalized) {
  ...
  inventoryState.hotbarSlots = normalized.hotbarSlots.map((item) => item ? { ...item } : null);
  inventoryState.equipment = {
    head: normalized.equipment.head ? { ...normalized.equipment.head } : null,
    backpack: normalized.equipment.backpack ? { ...normalized.equipment.backpack } : null,
  };
...
function serializeItemInventoryState() {
  return {
    ...
    hotbarSlots: inventoryState.hotbarSlots.map((item) => serializeInventoryItem(item)),
    equipment: {
      head: serializeInventoryItem(inventoryState.equipment.head),
      backpack: serializeInventoryItem(inventoryState.equipment.backpack),
    },
...
function findFirstItemByKind(kind) {
  for (const item of inventoryState.hotbarSlots) {
...
function applyItemLevelsToVisuals() {
  ...
  minerCharacter.setBackpackItem(getEquippedBackpack());
  ...
  if (inventoryPreviewCharacter) {
    ...
    inventoryPreviewCharacter.setBackpackItem(getEquippedBackpack());
  }
...
function findHotbarSlotByKind(kind) {
  for (let index = 0; index < HOTBAR_SLOT_COUNT; index++) {
    if (inventoryState.hotbarSlots[index]?.kind === kind) return index;
  }
  return -1;
}
...
function getLocationKey(location) {
  ...
  if (location.type === 'hotbar') return `hotbar:${location.index}`;
...
function cloneSlotLocation(location) {
  if (!location) return null;
  if (location.type === 'inventory' || location.type === 'hotbar' || location.type === 'chest') {
    return { type: location.type, index: location.index };
  }
  if (location.type === 'equipment') {
    return { type: 'equipment', slot: location.slot };
  }
...
function parseSlotLocationFromElement(element) {
  ...
  if (type === 'inventory') {
    return {
      type: 'inventory',
      index: Math.max(0, Math.min(inventoryState.itemSlots.length - 1, Math.floor(Number(slotElement.dataset.slotIndex)) || 0)),
    };
  }
  if (type === 'hotbar') {
    return {
      type: 'hotbar',
      index: Math.max(0, Math.min(HOTBAR_SLOT_COUNT - 1, Math.floor(Number(slotElement.dataset.slotIndex)) || 0)),
    };
  }
  if (type === 'chest') {
    return {
...
function getItemAtLocation(location) {
  ...
  if (location.type === 'hotbar') return inventoryState.hotbarSlots[location.index] ?? null;
...
function setItemAtLocation(location, item) {
  ...
  if (location.type === 'hotbar') {
    inventoryState.hotbarSlots[location.index] = item;
    return;
  }
...
function canPlaceItemAtLocation(item, location) {
  ...
  if (!item) return true;
  if (location.type === 'inventory') return location.index >= 0 && location.index < inventoryState.itemSlots.length;
  if (location.type === 'hotbar') return location.index >= 0 && location.index < HOTBAR_SLOT_COUNT;
  ...
  if (location.type === 'workbench') {
    return ['pickaxe', 'sword', 'crossbow', 'flashlight', 'backpack'].includes(item.kind);
  }
  return false;
}

function getItemAtLocationFromSnapshot(snapshot, location) {
  if (!snapshot || !location) return null;
  if (location.type === 'inventory') return snapshot.itemSlots[location.index] ?? null;
  if (location.type === 'hotbar') return snapshot.hotbarSlots[location.index] ?? null;
  if (location.type === 'chest') return snapshot.chestSlots[location.index] ?? null;
  if (location.type === 'equipment') return snapshot.equipment[location.slot] ?? null;
  if (location.type === 'workbench') return snapshot.workbenchItem ?? null;
  return null;
}

function setItemAtLocationInSnapshot(snapshot, location, item) {
  if (!snapshot || !location) return;
  if (location.type === 'inventory') {
    snapshot.itemSlots[location.index] = item;
    return;
  }
  if (location.type === 'hotbar') {
    snapshot.hotbarSlots[location.index] = item;
    return;
  }
  if (location.type === 'chest') {
    snapshot.chestSlots[location.index] = item;
    return;
  }
  if (location.type === 'equipment') {
    snapshot.equipment[location.slot] = item;
    return;
  }
  if (location.type === 'workbench') {
    snapshot.workbenchItem = item;
  }
}

function createInventoryMutationSnapshot() {
  return {
    itemSlots: inventoryState.itemSlots.map((item) => cloneInventoryEntry(item)),
    hotbarSlots: inventoryState.hotbarSlots.map((item) => cloneInventoryEntry(item)),
    chestSlots: chestState.itemSlots.map((item) => cloneInventoryEntry(item)),
    equipment: Object.fromEntries(Object.entries(inventoryState.equipment).map(([slot, item]) => [slot, cloneInventoryEntry(item)])),
    workbenchItem: cloneInventoryEntry(workbenchState.item),
  };
}

function getPackedInventoryItemsForCapacity(slots = inventoryState.itemSlots) {
  return (slots ?? []).filter(Boolean).map((item) => cloneInventoryEntry(item));
}

function canResolveInventorySnapshotCapacity(snapshot) {
  const capacity = getInventorySlotCapacityForEquipment(snapshot?.equipment);
  return Number.isFinite(capacity) && capacity >= 0;
}

function canResolveCurrentInventoryCapacity() {
  return canResolveInventorySnapshotCapacity(createInventoryMutationSnapshot());
}

function reconcileInventorySlotCapacity() {
  const capacity = getInventorySlotCapacity();
  if (!Number.isFinite(capacity) || capacity < 0) return { ok: false, moved: 0 };

  if (inventoryState.itemSlots.length < capacity) {
    while (inventoryState.itemSlots.length < capacity) inventoryState.itemSlots.push(null);
  ...
  const packedItems = getPackedInventoryItemsForCapacity(inventoryState.itemSlots);
  const visibleItems = packedItems.slice(0, capacity);
  const overflowItems = packedItems.slice(capacity);
  if (overflowItems.length) {
    dropInventoryItemsNearPlayer(overflowItems, { notice: true, scatterRadius: 0.42 });
  }
  inventoryState.itemSlots = Array.from({ length: capacity }, (_, index) => visibleItems[index] ? { ...visibleItems[index] } : null);
  return { ok: true, moved: overflowItems.length };
}

function placeHomeChestRecoveryItem(item) {
  const entry = cloneInventoryEntry(item);
  if (!entry) return 'none';

  if (entry.kind === 'backpack' && !inventoryState.equipment.backpack) {
    inventoryState.equipment.backpack = entry;
    reconcileInventorySlotCapacity();
    return 'equipment';
  }

  if (entry.kind === 'flashlight' && !inventoryState.equipment.head) {
    inventoryState.equipment.head = entry;
    itemState.flashlightOn = true;
    return 'equipment';
  }

  if (['pickaxe', 'sword'].includes(entry.kind)) {
    const hotbarIndex = inventoryState.hotbarSlots.findIndex((slotItem) => !slotItem);
    if (hotbarIndex !== -1) {
      inventoryState.hotbarSlots[hotbarIndex] = entry;
      return 'hotbar';
    }
  }

  const inventoryIndex = inventoryState.itemSlots.findIndex((slotItem) => !slotItem);
  if (inventoryIndex !== -1) {
    inventoryState.itemSlots[inventoryIndex] = entry;
    return 'inventory';
  }
  ...
  dropInventoryItemsNearPlayer([entry], { notice: false, scatterRadius: 0.26 });
  return 'ground';
}

function grantHomeChestRecoveryItem(kind) {
  if (areaState.currentArea !== 'home' || !HOME_CHEST_RECOVERY_ITEM_KINDS.includes(kind)) return false;
  if (hasItemKindAnywhere(kind)) return false;

  const rewardItem = createInventoryItem(kind, 1);
  const placement = placeHomeChestRecoveryItem(rewardItem);
  if (placement === 'none') return false;

  const itemName = getItemDisplayName(rewardItem);
  if (placement === 'ground') {
    setInventoryNotice(`${itemName} Lvl 1 caiu no chao porque nao havia espaco livre.`);
  } else {
    setInventoryNotice(`${itemName} Lvl 1 recuperado gratis no bau.`);
  }
...
function swapItemsBetweenLocations(source, target) {
  ...
  const previousState = createInventoryMutationSnapshot();
  const backpackMutation = (
    sourceItem?.kind === 'backpack'
    || targetItem?.kind === 'backpack'
    || source?.slot === 'backpack'
    || target?.slot === 'backpack'
  );
  setItemAtLocation(source, targetItem ? { ...targetItem } : null);
  setItemAtLocation(target, sourceItem ? { ...sourceItem } : null);
  const capacityResult = backpackMutation || inventoryState.itemSlots.length !== getInventorySlotCapacity()
    ? reconcileInventorySlotCapacity()
    : { ok: true, moved: 0 };
  if (!capacityResult.ok) {
    restoreInventoryMutationSnapshot(previousState);
    return false;
  }
  recalculateResourceTotals();
  applyItemLevelsToVisuals();
...
function dropItemFromLocation(source) {
  if (!canDropItemFromLocation(source)) return false;
  const sourceItem = cloneInventoryEntry(getItemAtLocation(source));
  if (!sourceItem) return false;
  setItemAtLocation(source, null);
  const capacityResult = source?.slot === 'backpack' || inventoryState.itemSlots.length !== getInventorySlotCapacity()
    ? reconcileInventorySlotCapacity()
    : { ok: true, moved: 0 };
  if (!capacityResult.ok) return false;
  dropInventoryItemsNearPlayer([sourceItem], { scatterRadius: 0.16, pickupDelay: INVENTORY_DROP_PICKUP_DELAY });
  recalculateResourceTotals();
  applyItemLevelsToVisuals();
  if (itemState.equippedSlot >= HOTBAR_SLOT_COUNT || itemState.equippedSlot < 0 || !getHotbarItem(itemState.equippedSlot)) {
    itemState.equippedSlot = -1;
...
function computeOreTotalsFromSlots(slotCollection = []) {
  const totals = makeEmptyTotals();
  for (const item of slotCollection) {
    if (item?.kind === 'ore' && item.oreId && totals[item.oreId] !== undefined) {
      totals[item.oreId] += item.amount;
    }
  }
  return totals;
}
...
function createDefaultUpgradeBalanceConfig() {
  ...
  return {
    ...
    backpack: {
      label: 'Mochila',
      maxLevel: BACKPACK_MAX_LEVEL,
      levels: levelsFromDefinitions(backpackDefinitions),
    },
  };
}
...
function applyUpgradeBalanceConfig() {
  ...
  for (const definition of backpackDefinitions) {
    if (definition.level > 1) Object.assign(definition, { cost: getUpgradeBalanceCost('backpack', definition.level) });
  }
}
...
function createDefaultGameplayBalanceConfig() {
  ...
  for (const definition of backpackDefinitions) {
    backpackLevels[String(definition.level)] = sanitizePositiveInteger(definition.extraSlots, 0, 0, 99);
  }
  for (const definition of oreDefinitions) {
...
function normalizeGameplayBalanceConfig(source = {}) {
  ...
  for (const definition of backpackDefinitions) {
    const key = String(definition.level);
    normalized.backpack[key] = sanitizePositiveInteger(source?.backpack?.[key], defaults.backpack[key], 0, 99);
  }
  for (const definition of oreDefinitions) {
...
function applyGameplayBalanceConfig() {
  ...
  for (const definition of backpackDefinitions) {
    definition.extraSlots = sanitizePositiveInteger(
      gameplayBalanceConfig.backpack[String(definition.level)],
      definition.extraSlots,
      0,
      99
    );
  }
...
function loadGame() {
  try {
    ...
    const overflowInventoryItems = collectOverflowInventoryItems(data.itemInventory);
    ...
    storeOverflowItemsInChest(overflowInventoryItems);
    if (!data.itemInventory && data.inventoryTotals) {
      const savedInventoryTotals = sanitizeTotals(data.inventoryTotals);
      const currentInventoryOreTotals = computeOreTotalsFromSlots(inventoryState.itemSlots);
      for (const definition of oreDefinitions) {
        const missingAmount = Math.max(0, (savedInventoryTotals[definition.id] ?? 0) - (currentInventoryOreTotals[definition.id] ?? 0));
        if (missingAmount > 0) addOreToChestItems(definition.id, missingAmount);
      }
    }
    recalculateResourceTotals();
    applyItemLevelsToVisuals();
...
function createItemIconMarkup(itemOrKind) {
  ...
  switch (kind) {
    ...
    case 'flashlight':
      ...
      `;
    case 'backpack': {
      const backpackDefinition = getBackpackDefinition(typeof itemOrKind === 'string' ? 1 : itemOrKind?.level ?? 1);
      const primary = formatHexColor(backpackDefinition.primary);
      const secondary = formatHexColor(backpackDefinition.secondary);
      const pocket = formatHexColor(backpackDefinition.pocket);
      const badge = formatHexColor(backpackDefinition.badge);
      const glow = formatHexColor(backpackDefinition.glow);
      const level = backpackDefinition.level;
      return `
        <svg viewBox="0 0 64 64" class="slot-icon slot-icon-backpack" aria-hidden="true">
          <ellipse cx="32" cy="36" rx="23" ry="18" fill="${glow}" opacity="${level >= 4 ? '0.2' : '0.1'}"/>
          <path d="M22 14c1.8-4.5 5-7 10-7s8.2 2.5 10 7h-5c-1.1-2-2.7-3-5-3s-3.9 1-5 3Z" fill="${secondary}"/>
          <rect x="17" y="14" width="30" height="38" rx="11" fill="${primary}"/>
          <rect x="20" y="17" width="24" height="33" rx="9" fill="${secondary}" opacity="0.42"/>
          <rect x="12.5" y="18" width="6" height="28" rx="3" fill="#26303a"/>
          <rect x="45.5" y="18" width="6" height="28" rx="3" fill="#26303a"/>
          <rect x="13.5" y="24" width="5" height="${Math.min(18, 8 + level * 3)}" rx="2.5" fill="${pocket}" opacity="0.95"/>
          <rect x="45.5" y="24" width="5" height="${Math.min(18, 8 + level * 3)}" rx="2.5" fill="${pocket}" opacity="0.95"/>
          <rect x="18" y="26" width="28" height="16" rx="7" fill="${pocket}"/>
          <rect x="22" y="30" width="20" height="12" rx="5" fill="${secondary}" opacity="0.55"/>
          <path d="M17 20c4 3.6 9.6 5.3 15 5.3S43 23.6 47 20" fill="none" stroke="rgba(255,255,255,0.35)" stroke-width="2.2" stroke-linecap="round"/>
          <path d="M22 24h20" fill="none" stroke="rgba(255,255,255,0.7)" stroke-width="2" stroke-linecap="round"/>
          ${Array.from({ length: level }, (_, index) => `<rect x="${29 + (index % 2) * 3}" y="${22 + index * 4.2}" width="2.2" height="3" rx="1.1" fill="#dfe7f1"/>`).join('')}
          ${level >= 3 ? `<circle cx="32" cy="37" r="${level >= 5 ? '5.3' : '4.5'}" fill="${badge}" opacity="0.98"/>` : ''}
          ${level >= 5 ? `<path d="m32 29.7 1.7 3.8 4.1.4-3.1 2.8.9 4-3.6-2.1-3.6 2.1.9-4-3.1-2.8 4.1-.4Z" fill="#fff6c8"/>` : ''}
        </svg>
      `;
    }
    default:
...
const INVENTORY_WORLD_ICON_STYLE = `
  .slot-icon-pickaxe .pickaxe-shadow { fill: rgba(0, 0, 0, 0.32); }
  .slot-icon-pickaxe .pickaxe-handle,
  .slot-icon-pickaxe .pickaxe-handle-tip { fill: #8f5f38; }
  .slot-icon-pickaxe .pickaxe-head { fill: #dce7f7; }
  .slot-icon-pickaxe .pickaxe-highlight { fill: rgba(255, 255, 255, 0.62); }
  .slot-icon-sword .sword-shadow { fill: rgba(0, 0, 0, 0.34); }
  .slot-icon-sword .sword-blade { fill: #dfeaff; }
  .slot-icon-sword .sword-core { fill: rgba(142, 197, 255, 0.78); }
  .slot-icon-sword .sword-guard { fill: #c59a4c; }
  .slot-icon-sword .sword-handle { fill: #6a4024; }
  .slot-icon-sword .sword-pommel { fill: #f0c766; }
  .slot-icon-crossbow .crossbow-shadow { fill: rgba(0, 0, 0, 0.34); }
  .slot-icon-crossbow .crossbow-limb { fill: #dcb76a; }
  .slot-icon-crossbow .crossbow-string { fill: rgba(224, 246, 255, 0.82); }
  .slot-icon-crossbow .crossbow-stock { fill: #6b4022; }
  .slot-icon-crossbow .crossbow-grip { fill: #4a2b16; }
  .slot-icon-crossbow .crossbow-bolt { fill: #d9eefb; }
  .slot-icon-crossbow .crossbow-tip { fill: #7fe8ff; }
`;

function buildInventoryWorldIconSvg(item) {
  const markup = createItemIconMarkup(item);
  if (!markup) return '';
  let svgMarkup = markup.trim();
  if (!/^<svg\b/i.test(svgMarkup)) return '';
  if (!/xmlns=/.test(svgMarkup)) {
    svgMarkup = svgMarkup.replace(/^<svg\b/i, '<svg xmlns="http://www.w3.org/2000/svg"');
  }
  return svgMarkup.replace(
    /^<svg\b([^>]*)>/i,
    `<svg$1><style>${INVENTORY_WORLD_ICON_STYLE}</style>`
  );
}

function formatItemLevelLabel(level) {
  const safeLevel = Math.max(1, Math.floor(Number(level)) || 1);
  return `Lvl ${safeLevel}`;
}
...
function getItemDefinition(item) {
  ...
  switch (item.kind) {
    ...
    case 'backpack': return getBackpackDefinition(item.level);
    default: return null;
  }
}
...
function getItemDisplayMeta(item) {
  ...
  switch (item.kind) {
    ...
    case 'backpack':
      return `+${definition.extraSlots} slots`;
    default:
      return '';
  }
}
...
function getNextUpgradeForItem(item) {
  ...
  switch (item.kind) {
    ...
    case 'backpack': {
      const nextLevel = item.level >= getUpgradeBalanceMaxLevel('backpack') ? null : item.level + 1;
      return nextLevel ? { ...getBackpackDefinition(nextLevel), cost: getUpgradeBalanceCost('backpack', nextLevel) } : null;
    }
    default:
      return null;
  }
...
function getInventoryWorldIconTexture(item) {
  if (!item) return null;
  const key = item.kind === 'ore'
    ? `ore:${item.oreId}`
    : item.kind === 'backpack'
      ? `backpack:${item.level}`
      : item.kind;
  if (inventoryWorldState.iconTextures.has(key)) {
    return inventoryWorldState.iconTextures.get(key);
  }
  const svgMarkup = buildInventoryWorldIconSvg(item);
  if (!svgMarkup) return null;
  const texture = inventoryTextureLoader.load(`data:image/svg+xml;charset=utf-8,${encodeURIComponent(svgMarkup)}`);
  texture.colorSpace = THREE.SRGBColorSpace;
  texture.anisotropy = 2;
  texture.minFilter = THREE.LinearFilter;
  texture.magFilter = THREE.LinearFilter;
  inventoryWorldState.iconTextures.set(key, texture);
  return texture;
}

function getInventoryWorldBadgeTexture(text) {
  const key = String(text ?? '');
  if (!key) return null;
  if (inventoryWorldState.badgeTextures.has(key)) {
    return inventoryWorldState.badgeTextures.get(key);
  }

  const fontSize = key.length >= 6 ? 34 : key.length >= 4 ? 36 : 41;
  const paddingX = key.length >= 6 ? 16 : 14;
  const paddingY = 10;
  const measureCanvas = document.createElement('canvas');
  const measureCtx = measureCanvas.getContext('2d');
  if (!measureCtx) return null;
  measureCtx.font = `700 ${fontSize}px Segoe UI`;
  const textWidth = Math.ceil(measureCtx.measureText(key).width);

  const canvas = document.createElement('canvas');
  canvas.width = Math.max(78, textWidth + paddingX * 2);
  ...
  inventoryWorldState.badgeTextures.set(key, texture);
  return texture;
}

function createInventoryWorldSlotRecord(location, {
  width = 0.28,
  height = 0.28,
  equipment = false,
} = {}) {
  const record = {
    location: cloneSlotLocation(location),
    width,
    height,
    equipment,
  };
  ...
  const backMaterial = new THREE.MeshStandardMaterial({
    color: equipment ? 0x31506d : 0x4a4a6a,
    metalness: 0.5,
    roughness: 0.48,
    transparent: equipment,
    opacity: equipment ? 0.92 : 1,
  });
  const faceMaterial = new THREE.MeshStandardMaterial({
    color: equipment ? 0x1a3a5a : 0x2a2a4a,
    metalness: equipment ? 0.42 : 0.28,
    roughness: equipment ? 0.6 : 0.72,
    transparent: equipment,
    opacity: equipment ? 0.84 : 1,
  });

  const back = new THREE.Mesh(
    new THREE.BoxGeometry(width + 0.06, height + 0.06, 0.12),
    backMaterial
  );
  ...
  const iconPlane = new THREE.Mesh(
    new THREE.PlaneGeometry(width * 0.68, height * 0.68),
    new THREE.MeshBasicMaterial({
      transparent: true,
      depthWrite: false,
      toneMapped: false,
  ...
  badge.scale.set(width * 0.72, height * 0.32, 1);
  badge.position.set(0, -height * 0.34, 0.12);
  badge.visible = false;
  group.add(badge);
  ...
  hitArea.userData.inventoryLocation = cloneSlotLocation(location);
  group.add(hitArea);

  record.group = group;
  record.back = back;
  record.face = face;
  record.iconPlane = iconPlane;
  record.badge = badge;
  record.hitArea = hitArea;
  inventoryWorldState.raycastables.push(hitArea);
  inventoryWorldState.slotRecords.push(record);
  inventoryWorldState.slotRecordMap.set(getLocationKey(location), record);
  return record;
}

function setInventoryWorldSlotVisual(record, item) {
  if (!record) return;
  const key = getLocationKey(record.location);
  ...
  const canSwapHere = canPlaceItemAtLocation(itemDragState.item, record.location) && canPlaceItemAtLocation(item, itemDragState.source);

  let faceColor = record.equipment ? 0x1a3a5a : 0x2a2a4a;
  let backColor = record.equipment ? 0x31506d : 0x4a4a6a;
  let emissiveColor = 0x000000;
  let emissiveIntensity = 0;

  if (item) {
    faceColor = record.equipment ? 0x27486b : 0x2a3a5a;
  }
  if (isSelected) {
    faceColor = 0x2f5b8f;
    backColor = 0x7cb3ff;
    emissiveColor = 0x2d6ebb;
  ...
  if (isDragTarget) {
    faceColor = canSwapHere ? 0x1d4b38 : 0x5a2222;
    backColor = canSwapHere ? 0x8ee6ae : 0xff8787;
    emissiveColor = canSwapHere ? 0x4bcf7a : 0xff4a4a;
    emissiveIntensity = 0.34;
  }

  record.face.material.color.setHex(faceColor);
  record.back.material.color.setHex(backColor);
  record.face.material.emissive.setHex(emissiveColor);
  record.face.material.emissiveIntensity = emissiveIntensity;

  if (!item) {
    record.iconPlane.visible = false;
    record.badge.visible = false;
    return;
  }

  const iconTexture = getInventoryWorldIconTexture(item);
  record.iconPlane.material.map = iconTexture;
  record.iconPlane.material.needsUpdate = true;
  record.iconPlane.visible = !!iconTexture;

  const badgeText = item.kind === 'ore'
    ? String(Math.max(1, Math.floor(Number(item.amount)) || 1))
    : formatItemLevelLabel(item.level);
  const badgeTexture = getInventoryWorldBadgeTexture(badgeText);
  if (!badgeTexture) {
    record.badge.visible = false;
    return;
  }
  const badgeAspect = Math.max(1, Number(badgeTexture.userData?.aspect) || 1);
  const badgeHeight = item.kind === 'ore' ? record.height * 0.23 : record.height * 0.25;
  const badgeWidth = Math.min(record.width * 0.82, badgeHeight * badgeAspect);
  record.badge.scale.set(badgeWidth, badgeHeight, 1);
  record.badge.position.set(
    record.width * 0.5 - badgeWidth * 0.5 - 0.018,
    -record.height * 0.5 + badgeHeight * 0.5 + 0.014,
    0.12
  );
  record.badge.material.map = badgeTexture;
  record.badge.material.needsUpdate = true;
  record.badge.visible = !!badgeTexture;
}

function clearInventoryWorldLayout() {
  if (inventoryWorldState.boardGroup) {
    inventoryWorldState.group.remove(inventoryWorldState.boardGroup);
    disposeGroup(inventoryWorldState.boardGroup);
    inventoryWorldState.boardGroup = null;
  }
  for (const record of [inventoryWorldState.headSlotRecord, inventoryWorldState.backpackSlotRecord]) {
    if (!record?.group) continue;
    record.group.parent?.remove(record.group);
    disposeGroup(record.group);
  }
  inventoryWorldState.headSlotRecord = null;
  inventoryWorldState.backpackSlotRecord = null;
  inventoryWorldState.slotRecords.length = 0;
  inventoryWorldState.slotRecordMap.clear();
  inventoryWorldState.raycastables.length = 0;
}

function rebuildInventoryWorldLayout() {
  clearInventoryWorldLayout();

  const slotCount = getInventorySlotCapacity();
  const columns = getInventoryGridColumnCount(slotCount);
  const rows = Math.ceil(slotCount / columns);
  const slotWidth = 0.28;
  const slotHeight = 0.28;
  const slotGap = 0.09;
  const paddingX = 0.09;
  const paddingY = 0.09;
  const gridWidth = columns * slotWidth + (columns - 1) * slotGap;
  const gridHeight = rows * slotHeight + (rows - 1) * slotGap;
  const panelWidth = gridWidth + paddingX * 2;
  const panelHeight = gridHeight + paddingY * 2;

  const boardGroup = new THREE.Group();
  const boardScale = 1.12;
  boardGroup.position.set(-1.18 - Math.max(0, panelWidth - 1.2) * 0.56, 0.94, 0.14);
  boardGroup.rotation.y = 0.08;
  boardGroup.scale.setScalar(boardScale);
  inventoryWorldState.group.add(boardGroup);
  inventoryWorldState.boardGroup = boardGroup;

  if (inventoryWorldState.boardLight) {
    inventoryWorldState.boardLight.position.set(boardGroup.position.x + 0.36, boardGroup.position.y + 0.94, 0.92);
  }
  if (inventoryWorldState.topLight) {
    inventoryWorldState.topLight.position.set(boardGroup.position.x + 0.62, boardGroup.position.y + 1.78, 0.64);
  }

  const panel = new THREE.Mesh(
    new THREE.BoxGeometry(panelWidth, panelHeight, 0.18),
    new THREE.MeshStandardMaterial({
      color: 0x1a1a3a,
      metalness: 0.48,
      roughness: 0.52,
  ...
  panel.position.set(panelWidth * 0.5, panelHeight * 0.5, -0.12);
  panel.castShadow = true;
  panel.receiveShadow = true;
  boardGroup.add(panel);

  const boardGlow = new THREE.Mesh(
    new THREE.PlaneGeometry(panelWidth + 0.04, panelHeight + 0.04),
    new THREE.MeshBasicMaterial({
      color: 0x27486b,
      transparent: true,
      opacity: 0.12,
  ...
  boardGlow.position.set(panelWidth * 0.5, panelHeight * 0.5, -0.02);
  boardGroup.add(boardGlow);

  for (let index = 0; index < slotCount; index++) {
    const row = Math.floor(index / columns);
    const col = index % columns;
    const x = paddingX + slotWidth * 0.5 + col * (slotWidth + slotGap);
    const y = panelHeight - paddingY - slotHeight * 0.5 - row * (slotHeight + slotGap);
    const record = createInventoryWorldSlotRecord({ type: 'inventory', index }, {
      width: slotWidth,
      height: slotHeight,
      equipment: false,
    });
    record.group.position.set(x, y, 0.05);
    boardGroup.add(record.group);
  }

  const headSlotRecord = createInventoryWorldSlotRecord({ type: 'equipment', slot: 'head' }, {
    width: slotWidth,
    height: slotHeight,
    equipment: true,
  });
  headSlotRecord.group.position.set(0.02, 2.12, 0.24);
  inventoryWorldState.group.add(headSlotRecord.group);
  inventoryWorldState.headSlotRecord = headSlotRecord;

  const backpackSlotRecord = createInventoryWorldSlotRecord({ type: 'equipment', slot: 'backpack' }, {
    width: slotWidth,
    height: slotHeight,
    equipment: true,
  });
  backpackSlotRecord.group.position.set(1.44, 1.42, 0.26);
  inventoryWorldState.group.add(backpackSlotRecord.group);
  inventoryWorldState.backpackSlotRecord = backpackSlotRecord;
  inventoryWorldState.inventorySlotCapacity = slotCount;
}

function ensureInventoryWorldBuilt() {
  if (!inventoryWorldState.built) {
    inventoryWorldState.group.visible = false;
    inventoryWorldState.group.name = 'inventory-world';
    playerRoot.add(inventoryWorldState.group);

    const lightGroup = new THREE.Group();
    lightGroup.position.set(0, 0, 0.8);
    inventoryWorldState.group.add(lightGroup);
    inventoryWorldState.lightGroup = lightGroup;

    const boardLight = new THREE.PointLight(0xf4f1ff, 2.8, 6.4, 2);
    lightGroup.add(boardLight);
    inventoryWorldState.boardLight = boardLight;

    const characterFillLight = new THREE.PointLight(0xb4d1ff, 2.35, 6.2, 2);
    characterFillLight.position.set(1.1, 1.72, 0.76);
    lightGroup.add(characterFillLight);

    const characterRimLight = new THREE.PointLight(0xfff4dd, 1.35, 4.8, 2);
    characterRimLight.position.set(0.68, 2.28, 0.42);
    lightGroup.add(characterRimLight);

    const topLight = new THREE.PointLight(0xffffff, 1.2, 4.8, 2);
    lightGroup.add(topLight);
    inventoryWorldState.topLight = topLight;

    inventoryWorldState.built = true;
  }

  if (!inventoryWorldState.boardGroup || inventoryWorldState.inventorySlotCapacity !== getInventorySlotCapacity()) {
    rebuildInventoryWorldLayout();
  }
}

function updateInventoryWorldDisplay() {
  if (!inventoryWorldState.built) return;
  for (const record of inventoryWorldState.slotRecords) {
    setInventoryWorldSlotVisual(record, getItemAtLocation(record.location));
  }
}

function getInventoryWorldLocationFromClient(clientX, clientY) {
  if (!inventoryWorldState.enabled || uiState.activePanel !== 'inventory' || !inventoryWorldState.raycastables.length) return null;
  const rect = renderer.domElement.getBoundingClientRect();
  if (!rect.width || !rect.height) return null;
  inventoryWorldPointer.set(
    ((clientX - rect.left) / rect.width) * 2 - 1,
    -((clientY - rect.top) / rect.height) * 2 + 1
  );
  raycaster.setFromCamera(inventoryWorldPointer, camera);
  raycastHits.length = 0;
  raycaster.intersectObjects(inventoryWorldState.raycastables, false, raycastHits);
  const hit = raycastHits.find((entry) => entry.object?.userData?.inventoryLocation);
  return hit ? cloneSlotLocation(hit.object.userData.inventoryLocation) : null;
}

function getInventoryCharacterHitFromClient(clientX, clientY) {
  if (!inventoryWorldState.enabled || uiState.activePanel !== 'inventory') return null;
  const rect = renderer.domElement.getBoundingClientRect();
  if (!rect.width || !rect.height) return null;
  inventoryWorldPointer.set(
    ((clientX - rect.left) / rect.width) * 2 - 1,
  ...
  minerCharacter.group.traverse((child) => {
    if (child.isMesh && child.visible) characterMeshes.push(child);
  });
  if (!characterMeshes.length) return null;
  raycaster.setFromCamera(inventoryWorldPointer, camera);
  raycastHits.length = 0;
  raycaster.intersectObjects(characterMeshes, true, raycastHits);
  return raycastHits[0] ?? null;
}

function captureInventoryPresentationSnapshot() {
  inventoryWorldState.snapshot = {
    lookYaw: lookState.yaw,
    lookPitch: lookState.pitch,
    characterRotationY: minerCharacter.group.rotation.y,
    characterPositionX: minerCharacter.group.position.x,
    cameraPosition: camera.position.clone(),
  };
}

function applyInventoryPresentationSnapshot() {
  if (!inventoryWorldState.snapshot) captureInventoryPresentationSnapshot();
  lookState.yaw = inventoryWorldState.snapshot.lookYaw + Math.PI;
  lookState.pitch = THREE.MathUtils.clamp(-0.08, -Math.PI * 0.48, Math.PI * 0.48);
  inventoryWorldState.characterYawOffset = 0;
  inventoryWorldState.characterZoom = 2.78;
  minerCharacter.group.position.x = 0.84;
  camera.position.set(0.24, 0.08, inventoryWorldState.characterZoom);
  applyLook();
}

function restoreInventoryPresentationSnapshot() {
  if (!inventoryWorldState.snapshot) return;
  lookState.yaw = inventoryWorldState.snapshot.lookYaw;
  lookState.pitch = inventoryWorldState.snapshot.lookPitch;
  minerCharacter.group.rotation.y = inventoryWorldState.snapshot.characterRotationY ?? Math.PI;
  minerCharacter.group.position.x = inventoryWorldState.snapshot.characterPositionX ?? 0;
  if (inventoryWorldState.snapshot.cameraPosition) camera.position.copy(inventoryWorldState.snapshot.cameraPosition);
  inventoryWorldState.characterYawOffset = 0;
  inventoryWorldState.characterZoom = 2.78;
  inventoryWorldState.characterDragActive = false;
  inventoryWorldState.characterDragPointerId = null;
  applyLook();
  inventoryWorldState.snapshot = null;
}

function updateInventoryWorldPresentation(delta) {
  if (!inventoryWorldState.enabled) {
    if (inventoryWorldState.group.visible) inventoryWorldState.group.visible = false;
    if (!devModeState.enabled && !buildModeState.enabled) {
      minerCharacter.group.rotation.y = Math.PI;
      minerCharacter.group.position.x = 0;
    }
    return;
  }

  ensureInventoryWorldBuilt();
  inventoryWorldState.group.visible = true;
  updateInventoryWorldDisplay();
  camera.position.x = THREE.MathUtils.lerp(camera.position.x, 0.24 + (inventoryWorldState.characterZoom - 2.78) * -0.08, 0.16);
  camera.position.y = THREE.MathUtils.lerp(camera.position.y, 0.08 + (2.78 - inventoryWorldState.characterZoom) * 0.03, 0.16);
  camera.position.z = THREE.MathUtils.lerp(camera.position.z, inventoryWorldState.characterZoom, 0.18);

  camera.getWorldPosition(inventoryWorldCameraPosition);
  minerCharacter.group.getWorldPosition(inventoryWorldCharacterPosition);
  inventoryWorldCameraPosition.y = inventoryWorldCharacterPosition.y;
  inventoryWorldCameraPosition.sub(inventoryWorldCharacterPosition);
  if (inventoryWorldCameraPosition.lengthSq() > 0.0001) {
    const worldYaw = Math.atan2(inventoryWorldCameraPosition.x, inventoryWorldCameraPosition.z);
    minerCharacter.group.rotation.y = worldYaw - playerRoot.rotation.y + inventoryWorldState.characterYawOffset;
  }
  if (inventoryWorldState.headSlotRecord?.group) {
    inventoryWorldState.headSlotRecord.group.position.x = minerCharacter.group.position.x + 0.02;
  }
  if (inventoryWorldState.backpackSlotRecord?.group) {
    inventoryWorldState.backpackSlotRecord.group.position.x = minerCharacter.group.position.x + 0.62;
  }

}

function beginInventoryCharacterDrag(event) {
  if (event.button !== 0) return;
  if (!inventoryWorldState.enabled || uiState.activePanel !== 'inventory') return;
  if (devModeState.enabled || buildModeState.enabled || itemDragState.active) return;
  if (getInventoryWorldLocationFromClient(event.clientX, event.clientY)) return;
  const characterHit = getInventoryCharacterHitFromClient(event.clientX, event.clientY);
  if (!characterHit) return;
  inventoryWorldState.characterDragActive = true;
  inventoryWorldState.characterDragPointerId = event.pointerId ?? null;
  inventoryWorldState.characterDragLastX = event.clientX;
  try {
...
function updateInventoryCharacterDrag(event) {
  if (!inventoryWorldState.characterDragActive) return;
  if (inventoryWorldState.characterDragPointerId !== null && event.pointerId !== inventoryWorldState.characterDragPointerId) return;
  const deltaX = event.clientX - inventoryWorldState.characterDragLastX;
  inventoryWorldState.characterDragLastX = event.clientX;
  inventoryWorldState.characterYawOffset += deltaX * inventoryWorldState.characterDragSensitivity;
}

function endInventoryCharacterDrag(event = null) {
  if (!inventoryWorldState.characterDragActive) return;
  if (
    event
    && inventoryWorldState.characterDragPointerId !== null
    && event.pointerId !== undefined
    && event.pointerId !== inventoryWorldState.characterDragPointerId
  ) {
    return;
  }
  try {
    if (
      inventoryWorldState.characterDragPointerId !== null
      && renderer.domElement.hasPointerCapture?.(inventoryWorldState.characterDragPointerId)
    ) {
      renderer.domElement.releasePointerCapture(inventoryWorldState.characterDragPointerId);
    }
  } catch (error) {
    void error;
  }
...
function zoomInventoryCharacter(deltaY) {
  inventoryWorldState.characterZoom = THREE.MathUtils.clamp(
    inventoryWorldState.characterZoom + deltaY * 0.0026,
    1.7,
    4.2
  );
}
...
const world = {
    ...
    function createMinerCharacter() {
      ...
      const backpackRoot = new THREE.Group();
      backpackRoot.position.set(0, 1.08, -0.2);
      backpackRoot.visible = false;
      model.add(backpackRoot);
      let backpackModel = null;
      let backpackModelSignature = '';

      function createCharacterBackpack(level = 1) {
        const definition = getBackpackDefinition(level);
        const safeLevel = definition.level;
        const group = new THREE.Group();
        group.position.set(0, -0.16, 0);
        group.scale.setScalar(0.18);
        ...
        const seenEmissiveMaterials = new Set();
        const seenOpacityMaterials = new Set();

        const addShadowMesh = (geometry, material, px, py, pz, rx = 0, ry = 0, rz = 0) => {
          const mesh = new THREE.Mesh(geometry, material);
          mesh.position.set(px, py, pz);
          mesh.rotation.set(rx, ry, rz);
          mesh.castShadow = true;
        ...
        const createRoundedBoxGeometry = (width, height, depth, radius, segments = 3) => {
          const shape = new THREE.Shape();
          const halfWidth = width / 2;
          const halfHeight = height / 2;
          shape.moveTo(-halfWidth + radius, -halfHeight);
        ...
        const registerEmissivePulse = (material, amplitude = 0.24, speed = 2.2) => {
          if (!material || material.emissiveIntensity === undefined || seenEmissiveMaterials.has(material)) return material;
          seenEmissiveMaterials.add(material);
          emissiveTargets.push({
            material,
            base: material.emissiveIntensity,
            amplitude,
            speed,
            phase: emissiveTargets.length * 0.68,
          });
          return material;
        };

        const registerOpacityPulse = (material, amplitude = 0.08, speed = 2.2) => {
          if (!material || material.opacity === undefined || seenOpacityMaterials.has(material)) return material;
          seenOpacityMaterials.add(material);
          opacityTargets.push({
            material,
            base: material.opacity,
            amplitude,
            speed,
            phase: opacityTargets.length * 0.82,
          });
          return material;
        };

        const registerSpin = (mesh, speed = 1.2) => {
          if (!mesh) return mesh;
          rotatingTargets.push({ mesh, speed });
          return mesh;
        };

        const addHalo = (width, height, color, opacity, px, py, pz, ry = 0) => {
          ...
          registerOpacityPulse(material, opacity * 0.32, 2.4);
        ...
        const createParticles = (color, count, radius, { size = 0.04, opacity = 0.7 } = {}) => {
          const positions = new Float32Array(count * 3);
          const velocities = [];
          for (let index = 0; index < count; index++) {
            const theta = Math.random() * Math.PI * 2;
          ...
          particleTargets.push({
            ...
            velocities,
            radius,
            spinSpeed: 0.1 + particleTargets.length * 0.04,
          });
          return points;
        };
        ...
        if (safeLevel === 1) {
          ...
          const strapMat = new THREE.MeshStandardMaterial({ color: 0x4a2e15, roughness: 0.75, metalness: 0.05 });
          const buckleMat = new THREE.MeshStandardMaterial({ color: 0x8a7a5a, roughness: 0.4, metalness: 0.7 });
          const flapMat = new THREE.MeshStandardMaterial({ color: 0x5a3520, roughness: 0.8, metalness: 0.05 });
          const stitchMat = new THREE.MeshBasicMaterial({ color: 0x8b7355, toneMapped: false });

          addShadowMesh(createRoundedBoxGeometry(1.1, 1.3, 0.6, 0.08), bodyMat, 0, 0.9, 0);
          addShadowMesh(createRoundedBoxGeometry(1.14, 0.4, 0.06, 0.05), flapMat, 0, 1.55, -0.22, -0.25, 0, 0);
          addShadowMesh(new THREE.TorusGeometry(0.06, 0.015, 8, 12), buckleMat, 0, 1.35, -0.33);

          for (const side of [-1, 1]) {
            addShadowMesh(new THREE.BoxGeometry(0.1, 1.4, 0.04), strapMat, side * 0.35, 0.8, 0.32);
            addShadowMesh(new THREE.TorusGeometry(0.1, 0.025, 8, 12, Math.PI), strapMat, side * 0.35, 1.5, 0.22, 0, Math.PI / 2, 0);
          }

          for (const y of [0.45, 1.35]) {
            const stitch = new THREE.Mesh(new THREE.BoxGeometry(0.95, 0.015, 0.005), stitchMat);
            stitch.position.set(0, y, -0.31);
            group.add(stitch);
          }
        } else if (safeLevel === 2) {
          ...
          const strapMat = new THREE.MeshStandardMaterial({ color: 0x4a4a3a, roughness: 0.65, metalness: 0.15 });
          const metalMat = new THREE.MeshStandardMaterial({ color: 0x777766, roughness: 0.3, metalness: 0.8 });
          const bottomMat = new THREE.MeshStandardMaterial({ color: 0x2a2a22, roughness: 0.6, metalness: 0.2 });
          const rollMat = new THREE.MeshStandardMaterial({ color: 0x6a6a55, roughness: 0.8 });

          addShadowMesh(createRoundedBoxGeometry(1.2, 1.45, 0.7, 0.08), bodyMat, 0, 0.95, 0);
          addShadowMesh(createRoundedBoxGeometry(0.8, 0.5, 0.15, 0.05), accentMat, 0, 0.6, -0.38);
          for (const side of [-1, 1]) {
            addShadowMesh(createRoundedBoxGeometry(0.15, 0.45, 0.4, 0.04), accentMat, side * 0.65, 0.6, 0);
          }
          addShadowMesh(createRoundedBoxGeometry(1.24, 0.45, 0.07, 0.05), accentMat, 0, 1.68, -0.28, -0.2, 0, 0);
          for (const x of [-0.25, 0.25]) {
            addShadowMesh(new THREE.BoxGeometry(0.08, 0.12, 0.03), metalMat, x, 1.42, -0.37);
          }
          for (const side of [-1, 1]) {
            addShadowMesh(new THREE.BoxGeometry(0.13, 1.5, 0.05), strapMat, side * 0.38, 0.85, 0.38);
            addShadowMesh(createRoundedBoxGeometry(0.16, 0.4, 0.07, 0.03), strapMat, side * 0.38, 0.95, 0.4);
          }
          addShadowMesh(createRoundedBoxGeometry(1.22, 0.15, 0.72, 0.03), bottomMat, 0, 0.23, 0);
          addShadowMesh(new THREE.CylinderGeometry(0.12, 0.12, 1.0, 16), rollMat, 0, 1.85, 0.1, 0, 0, Math.PI / 2);
        } else if (safeLevel === 3) {
          const bodyMat = new THREE.MeshStandardMaterial({ color: 0x2a4466, roughness: 0.55, metalness: 0.25 });
          const panelMat = new THREE.MeshStandardMaterial({ color: 0x1e3350, roughness: 0.5, metalness: 0.3 });
          const metalMat = new THREE.MeshStandardMaterial({ color: 0x99aabb, roughness: 0.2, metalness: 0.9 });
          const glowMat = registerEmissivePulse(new THREE.MeshStandardMaterial({
            color: 0x44aaff,
            roughness: 0.3,
            metalness: 0.5,
            emissive: 0x2266aa,
          ...
          const capMat = registerEmissivePulse(new THREE.MeshStandardMaterial({
            color: 0x44aaff,
            metalness: 0.8,
            roughness: 0.2,
            emissive: 0x2266aa,
          ...
          const strapMat = new THREE.MeshStandardMaterial({ color: 0x333d4d, roughness: 0.6, metalness: 0.2 });
          const padMat = new THREE.MeshStandardMaterial({ color: 0x2a4466, roughness: 0.5, metalness: 0.3 });
          const pickHandleMat = new THREE.MeshStandardMaterial({ color: 0x6a5030, roughness: 0.8 });

          addShadowMesh(createRoundedBoxGeometry(1.3, 1.55, 0.75, 0.06, 4), bodyMat, 0, 1.0, 0);
          [
            { pos: [0, 1.78, 0], size: [1.34, 0.04, 0.79] },
            { pos: [0, 0.22, 0], size: [1.34, 0.04, 0.79] },
            { pos: [-0.67, 1.0, 0], size: [0.04, 1.6, 0.79] },
            ...
            addShadowMesh(new THREE.BoxGeometry(...frame.size), metalMat, ...frame.pos);
          });

          for (let index = 0; index < 2; index++) {
            const x = index === 0 ? -0.28 : 0.28;
            addShadowMesh(createRoundedBoxGeometry(0.5, 0.45, 0.12, 0.03), panelMat, x, 0.6, -0.42);
            const light = addShadowMesh(new THREE.SphereGeometry(0.025, 12, 12), glowMat, x, 0.82, -0.48);
            registerSpin(light, 0.7);
          }

          addShadowMesh(createRoundedBoxGeometry(1.1, 0.3, 0.12, 0.04), panelMat, 0, 1.5, -0.4);

          for (const side of [-1, 1]) {
            addShadowMesh(new THREE.CylinderGeometry(0.1, 0.1, 0.5, 12), metalMat, side * 0.72, 0.7, 0);
            addShadowMesh(new THREE.CylinderGeometry(0.11, 0.11, 0.04, 12), capMat, side * 0.72, 0.96, 0);
            addShadowMesh(new THREE.BoxGeometry(0.14, 1.55, 0.05), strapMat, side * 0.4, 0.9, 0.4);
            for (const py of [0.5, 0.9, 1.3]) {
              addShadowMesh(createRoundedBoxGeometry(0.18, 0.15, 0.08, 0.02), padMat, side * 0.4, py, 0.43);
            }
          }

          addShadowMesh(new THREE.CylinderGeometry(0.025, 0.025, 0.9, 8), pickHandleMat, 0.82, 1.1, 0.1, 0, 0, 0.15);
          addShadowMesh(new THREE.BoxGeometry(0.3, 0.06, 0.06), metalMat, 0.82, 1.55, 0.1);
        } else if (safeLevel === 4) {
          const bodyMat = new THREE.MeshStandardMaterial({ color: 0x3a1a4a, roughness: 0.45, metalness: 0.35 });
          const trimMat = new THREE.MeshStandardMaterial({ color: 0xc8a030, roughness: 0.2, metalness: 0.9 });
          const flapMat = new THREE.MeshStandardMaterial({ color: 0x2a1038, roughness: 0.4, metalness: 0.35 });
          const sidePanelMat = new THREE.MeshStandardMaterial({ color: 0x2a1038, roughness: 0.5, metalness: 0.3 });
          const strapMat = new THREE.MeshStandardMaterial({ color: 0x2a1535, roughness: 0.5, metalness: 0.3 });
          const crystalMat = registerEmissivePulse(new THREE.MeshStandardMaterial({
            color: 0xaa44ff,
            roughness: 0.1,
            metalness: 0.3,
            emissive: 0x6622aa,
          ...
          const runeMat = registerEmissivePulse(new THREE.MeshStandardMaterial({
            color: 0xdd88ff,
            emissive: 0xaa44dd,
          ...
          addShadowMesh(createRoundedBoxGeometry(1.3, 1.6, 0.75, 0.07, 5), bodyMat, 0, 1.05, 0);
          [
            { pos: [0, 1.86, 0], size: [1.36, 0.05, 0.81] },
            { pos: [0, 0.24, 0], size: [1.36, 0.05, 0.81] },
            { pos: [-0.68, 1.05, 0], size: [0.05, 1.67, 0.81] },
            ...
            addShadowMesh(new THREE.BoxGeometry(...trim.size), trimMat, ...trim.pos);
          });

          addShadowMesh(createRoundedBoxGeometry(1.34, 0.5, 0.08, 0.05), flapMat, 0, 1.82, -0.28, -0.18, 0, 0);
          const mainCrystal = addShadowMesh(new THREE.OctahedronGeometry(0.15, 0), crystalMat, 0, 1.05, -0.42, 0, Math.PI / 4, 0);
          registerSpin(mainCrystal, 1.2);
          addHalo(0.52, 0.52, 0xaa44ff, 0.16, 0, 1.05, -0.48);

          const runeCircleGeo = new THREE.TorusGeometry(0.2, 0.012, 8, 32);
          for (let index = 0; index < 3; index++) {
            const angle = (index / 3) * Math.PI * 2;
            addShadowMesh(runeCircleGeo, runeMat, Math.cos(angle) * 0.35, 0.7 + Math.sin(angle) * 0.3, -0.4, 0, 0.1, 0);
          }

          const cornerPositions = [[-0.55, 1.7, -0.35], [0.55, 1.7, -0.35], [-0.55, 0.4, -0.35], [0.55, 0.4, -0.35]];
          for (const position of cornerPositions) {
            const crystal = addShadowMesh(new THREE.OctahedronGeometry(0.06, 0), crystalMat, ...position, 0, Math.PI / 4, 0);
            registerSpin(crystal, 1.6);
          }

          for (const side of [-1, 1]) {
            addShadowMesh(createRoundedBoxGeometry(0.06, 0.8, 0.5, 0.02), sidePanelMat, side * 0.7, 0.8, 0);
            addShadowMesh(new THREE.TorusGeometry(0.1, 0.01, 6, 20), runeMat, side * 0.74, 0.8, 0, 0, Math.PI / 2, 0);
            addShadowMesh(new THREE.BoxGeometry(0.12, 1.6, 0.04), strapMat, side * 0.4, 0.9, 0.4);
            for (const py of [0.4, 1.4]) {
              addShadowMesh(new THREE.TorusGeometry(0.04, 0.012, 6, 12), trimMat, side * 0.4, py, 0.43);
            }
          }

          createParticles(0xaa44ff, 80, 1.5, { size: 0.038, opacity: 0.62 });
        } else {
          const bodyMat = new THREE.MeshStandardMaterial({ color: 0x1a1215, roughness: 0.35, metalness: 0.5 });
          const goldMat = registerEmissivePulse(new THREE.MeshStandardMaterial({
            color: 0xffd700,
            roughness: 0.15,
            metalness: 0.95,
            emissive: 0x443300,
          ...
          const runeGoldMat = registerEmissivePulse(new THREE.MeshStandardMaterial({
            color: 0xffd700,
            emissive: 0xff8800,
          ...
          const darkCrystalMat = registerEmissivePulse(new THREE.MeshStandardMaterial({
            color: 0xff3300,
            roughness: 0.1,
            metalness: 0.3,
            emissive: 0xff2200,
          ...
          const flapMat = new THREE.MeshStandardMaterial({ color: 0x1a1015, roughness: 0.35, metalness: 0.5 });
          const panelMat = new THREE.MeshStandardMaterial({ color: 0x15100d, roughness: 0.4, metalness: 0.4 });
          const strapMat = new THREE.MeshStandardMaterial({ color: 0x1a1215, roughness: 0.4, metalness: 0.4 });
          const strapTrimMat = new THREE.MeshStandardMaterial({ color: 0xaa8820, roughness: 0.3, metalness: 0.8 });

          addShadowMesh(createRoundedBoxGeometry(1.4, 1.7, 0.8, 0.06, 5), bodyMat, 0, 1.1, 0);
          addShadowMesh(new THREE.BoxGeometry(1.46, 0.06, 0.86), goldMat, 0, 1.96, 0);
          addShadowMesh(new THREE.BoxGeometry(1.46, 0.06, 0.86), goldMat, 0, 0.24, 0);
          addShadowMesh(new THREE.BoxGeometry(0.06, 1.78, 0.86), goldMat, -0.73, 1.1, 0);
          addShadowMesh(new THREE.BoxGeometry(0.06, 1.78, 0.86), goldMat, 0.73, 1.1, 0);

          for (const dx of [-0.35, 0.35]) {
            for (const dy of [0.65, 1.45]) {
              addShadowMesh(new THREE.BoxGeometry(0.25, 0.025, 0.02), goldMat, dx, dy, -0.42, 0, 0, Math.PI / 4);
              addShadowMesh(new THREE.BoxGeometry(0.025, 0.25, 0.02), goldMat, dx, dy, -0.42, 0, 0, Math.PI / 4);
            }
          }

          const mainCrystal = addShadowMesh(new THREE.OctahedronGeometry(0.2, 1), crystalMat, 0, 1.1, -0.45, 0, Math.PI / 4, 0);
          registerSpin(mainCrystal, 1.35);
          addShadowMesh(new THREE.TorusGeometry(0.25, 0.025, 12, 32), goldMat, 0, 1.1, -0.43);
          addShadowMesh(new THREE.TorusGeometry(0.15, 0.015, 8, 24), runeGoldMat, 0, 1.1, -0.44);
          addHalo(0.64, 0.64, 0xff8800, 0.2, 0, 1.1, -0.52);

          const gemColors = [0xff2200, 0xff6600, 0xffaa00, 0xff2200, 0xff6600, 0xffaa00];
          for (let index = 0; index < 6; index++) {
            const angle = (index / 6) * Math.PI * 2;
            const gemMat = registerEmissivePulse(new THREE.MeshStandardMaterial({
              color: gemColors[index],
              emissive: gemColors[index],
              emissiveIntensity: 0.8,
              transparent: true,
            ...
            registerSpin(gem, 1.7);
          }

          addShadowMesh(new THREE.CylinderGeometry(0.3, 0.35, 0.08, 6), goldMat, 0, 1.98, -0.1);
          for (let index = 0; index < 5; index++) {
            const angle = (index / 5) * Math.PI * 2 - Math.PI / 2;
            addShadowMesh(new THREE.ConeGeometry(0.04, 0.2, 6), goldMat, Math.cos(angle) * 0.22, 2.12, -0.1 + Math.sin(angle) * 0.22);
          }
          const topCrystal = addShadowMesh(new THREE.OctahedronGeometry(0.08, 0), darkCrystalMat, 0, 2.22, -0.1);
          registerSpin(topCrystal, 1.9);
          addHalo(0.28, 0.28, 0xff4400, 0.14, 0, 2.22, -0.16);

          addShadowMesh(createRoundedBoxGeometry(1.42, 0.35, 0.06, 0.04), flapMat, 0, 1.9, -0.32, -0.15, 0, 0);

          for (const side of [-1, 1]) {
            addShadowMesh(createRoundedBoxGeometry(0.08, 1.0, 0.55, 0.02), panelMat, side * 0.77, 0.9, 0);
            for (let index = 0; index < 4; index++) {
              addShadowMesh(new THREE.TorusGeometry(0.035, 0.01, 6, 10), goldMat, side * 0.82, 0.5 + index * 0.22, -0.2, index % 2 === 0 ? 0 : Math.PI / 2, 0, 0);
            }
            addShadowMesh(new THREE.TorusGeometry(0.12, 0.012, 8, 24), runeGoldMat, side * 0.82, 0.9, 0.05, 0, Math.PI / 2, 0);
            addShadowMesh(new THREE.BoxGeometry(0.14, 1.7, 0.04), strapMat, side * 0.42, 0.95, 0.42);
            addShadowMesh(new THREE.BoxGeometry(0.16, 1.72, 0.01), strapTrimMat, side * 0.42, 0.95, 0.45);
            for (const py of [0.35, 0.95, 1.55]) {
              addShadowMesh(new THREE.TorusGeometry(0.05, 0.015, 8, 16), goldMat, side * 0.42, py, 0.46);
            }
          }

          createParticles(0xffaa00, 150, 2.0, { size: 0.042, opacity: 0.74 });
          createParticles(0xff3300, 60, 1.8, { size: 0.038, opacity: 0.62 });
        }

        group.userData.update = (elapsedTime, delta = 0.016) => {
          const safeDelta = Math.max(0.001, Number(delta) || 0.016);
          for (const target of emissiveTargets) {
            target.material.emissiveIntensity = target.base + Math.sin(elapsedTime * target.speed + target.phase) * target.amplitude;
          }
          for (const target of opacityTargets) {
            target.material.opacity = Math.max(0, target.base + Math.sin(elapsedTime * target.speed + target.phase) * target.amplitude);
          }
          for (const target of rotatingTargets) {
            target.mesh.rotation.y += target.speed * safeDelta;
          }
          for (const particleSystem of particleTargets) {
            const positions = particleSystem.positions;
            for (let index = 0; index < particleSystem.velocities.length; index++) {
              ...
              if (
                positions[index * 3 + 1] > 3.0
                || Math.abs(positions[index * 3]) > particleSystem.radius
                || Math.abs(positions[index * 3 + 2]) > particleSystem.radius
              ) {
                positions[index * 3] = (Math.random() - 0.5) * 1.0;
                positions[index * 3 + 1] = 0.2 + Math.random() * 0.3;
                positions[index * 3 + 2] = (Math.random() - 0.5) * 1.0;
            ...
            particleSystem.points.rotation.y = elapsedTime * particleSystem.spinSpeed;
          }
        };
      ...
      function setBackpackItem(item) {
        const nextSignature = item?.kind === 'backpack'
          ? `${item.id ?? 'backpack'}:${item.level ?? 1}`
          : '';
        if (nextSignature && backpackModel && backpackModelSignature === nextSignature) {
          backpackRoot.visible = true;
          return;
        }
        if (backpackModel) {
          backpackRoot.remove(backpackModel);
          disposeGroup(backpackModel);
          backpackModel = null;
        }
        if (item?.kind !== 'backpack') {
          backpackModelSignature = '';
          backpackRoot.visible = false;
          return;
        }
        backpackModel = createLiteralHtmlBackpackModel(item.level);
        backpackRoot.add(backpackModel);
        backpackModelSignature = nextSignature;
        backpackRoot.visible = true;
      }
      ...
      setBackpackItem(getEquippedBackpack());
      ...
      return {
        ...
        update(delta, mode, pickaxeVisible, swordVisible, crossbowVisible = false) {
          ...
          backpackModel?.userData?.update?.(clock.elapsedTime, delta);
          resetPose();
    ...
    function updateInventoryPreview(delta) {
      if (!inventoryPreviewCanvas || uiState.activePanel !== 'inventory' || !inventoryPanel?.classList.contains('visible')) return;
      ensureInventoryPreviewReady();
    ...
    function getGameplayBalanceInputCurrentValue(input) {
      ...
      if (gameplayKind === 'backpack') return gameplayBalanceConfig.backpack[String(input.dataset.level)] ?? 0;
    ...
    function renderDevGameplayEditor() {
      ...
      if (devModeState.section === 'player') {
        ...
        const backpackCard = createDevGameplayCard('Mochila: slots extras por nivel', 'player');
        for (const definition of backpackDefinitions) {
          backpackCard.grid.append(
            createDevGameplayRow(
              `Nivel ${definition.level}`,
              'Slots extras ao equipar a mochila',
              gameplayBalanceConfig.backpack[String(definition.level)] ?? definition.extraSlots,
              { gameplayKind: 'backpack', level: definition.level },
              { min: 0, max: 99, step: 1 }
            )
          );
        }
        devGameplayControls.append(backpackCard.card);
      }
    ...
    function setGameplayBalanceValue(input) {
      ...
      } else if (input.dataset.gameplayKind === 'backpack') {
        gameplayBalanceConfig.backpack[String(input.dataset.level)] = THREE.MathUtils.clamp(value, 0, 99);
      ...
      gameplayBalanceConfig = normalizeGameplayBalanceConfig(gameplayBalanceConfig);
      applyGameplayBalanceConfig();
      let backpackSlotsDropped = 0;
      if (input.dataset.gameplayKind === 'backpack') {
        backpackSlotsDropped = reconcileInventorySlotCapacity().moved;
      }
      refreshResourceUi();
      updateMiningHud();
      ...
      input.value = valueType === 'float'
        ...
        : String(appliedValue);
      setDevStatus(
        backpackSlotsDropped > 0
          ? 'Valor atualizado em tempo real. Os itens excedentes da mochila cairam no chao.'
          : 'Valor atualizado em tempo real. Clique em Salvar configuracao para persistir.'
      );
    }
    ...
    function resetGameplayBalanceConfig() {
      gameplayBalanceConfig = clonePlain(gameplayBalanceDefaults);
      applyGameplayBalanceConfig();
      const backpackSlotsDropped = reconcileInventorySlotCapacity().moved;
      renderDevGameplayEditor();
      refreshResourceUi();
      updateMiningHud();
      setDevStatus(
        backpackSlotsDropped > 0
          ? 'Valores do jogo resetados. Os itens excedentes da mochila cairam no chao.'
          : 'Valores do jogo resetados. Clique em Salvar configuracao para persistir.'
      );
    }
    ...
    renderer.domElement.addEventListener('pointerdown', (event) => {
      ...
      else beginInventoryCharacterDrag(event);
    });
    ...
    renderer.domElement.addEventListener('wheel', (event) => {
      if (inventoryWorldState.enabled && uiState.activePanel === 'inventory') {
        zoomInventoryCharacter(event.deltaY);
        event.preventDefault();
        return;
      }
      if (devModeState.enabled) {
    ...
    function getHotbarBadgeText(item) {
      if (!item) return '';
      return item.kind === 'ore'
        ? `x${item.amount}`
        : formatItemLevelLabel(item.level);
    ...
    function updateHotbar() {
      hotbarSlots.forEach((slot, index) => {
        ...
        const location = { type: 'hotbar', index };
        ...
        slot.dataset.locationType = 'hotbar';
        ...
        slot.innerHTML = item ? `
          ...
          <span class="slot-icon-shell" aria-hidden="true">${createItemIconMarkup(item)}</span>
          <span class="slot-badge ${item.kind === 'ore' ? 'item-stack-count' : ''}">${getHotbarBadgeText(item)}</span>
        ` : `
          <span class="slot-num">${index + 1}</span>
          <span class="slot-icon-shell" aria-hidden="true"></span>
        `;
      });
    }
    ...
    function updateCombatHud() {
      ...
      if (equippedItem?.kind === 'crossbow') {
        swordHudText.textContent = `Crossbow ${formatItemLevelLabel(crossbowDefinition.level)} | Dano ${crossbowDefinition.damage} | Alcance ${crossbowDefinition.range}`;
      } else if (equippedItem?.kind === 'sword') {
        swordHudText.textContent = `Espada ${formatItemLevelLabel(swordDefinition.level)} | Dano ${swordDefinition.damage}`;
      } else if (equippedItem?.kind === 'pickaxe') {
        swordHudText.textContent = `Picareta ${formatItemLevelLabel(getPickaxeDefinition().level)} | Dano ${getPickaxeDefinition().damage}`;
        ...
        swordHudText.textContent = `Lanterna ${formatItemLevelLabel(playerStats.flashlightLevel)} | Alcance ${flashlightStats.distance.toFixed(0)}`;
      } else if (equippedItem?.kind === 'backpack') {
        const backpackDefinition = getBackpackDefinition(equippedItem.level);
        swordHudText.textContent = `Mochila ${formatItemLevelLabel(backpackDefinition.level)} | +${backpackDefinition.extraSlots} slots`;
      } else {
    ...
    function beginItemDrag(event) {
      ...
      const worldSlotLocation = getInventoryWorldLocationFromClient(event.clientX, event.clientY);
      if (!event.target.closest('#inventoryLayout, #upgradePanel, #chestPanel, #hotbar') && !worldSlotLocation) return;
      const slotLocation = parseSlotLocationFromElement(event.target) ?? worldSlotLocation;
      ...
      if (itemDragGhost) {
        ...
        itemDragGhost.innerHTML = buildItemSlotMarkup(slotLocation, item, {
          shortcutLabel: slotLocation.type === 'hotbar' ? String(slotLocation.index + 1) : '',
          selected: slotLocation.type === 'hotbar' && slotLocation.index === itemState.equippedSlot,
        });
      }
...
function updateItemDragTarget(clientX, clientY) {
  ...
  itemDragState.target = parseSlotLocationFromElement(targetElement) ?? getInventoryWorldLocationFromClient(clientX, clientY);
  updateItemDragGhostPosition(clientX, clientY);
  renderInventoryPanels();
}

function isExternalInventoryDropRelease(clientX, clientY) {
  const targetElement = document.elementFromPoint(clientX, clientY);
  if (targetElement?.closest?.('#inventoryLayout, #upgradePanel, #chestPanel, #hotbar')) return false;
  return !getInventoryWorldLocationFromClient(clientX, clientY);
}
...
function endItemDrag(event) {
  ...
  if (source && target && getLocationKey(source) !== getLocationKey(target)) {
    ...
    && canDropItemFromLocation(source)
    && isExternalInventoryDropRelease(event.clientX, event.clientY)
  ) {
    changed = dropItemFromLocation(source);
  }
  ...
  if (changed) {
    ...
    renderer.domElement.addEventListener('pointerdown', beginItemDrag);
    ...
    renderer.domElement.addEventListener('click', (event) => {
      if (uiState.activePanel !== 'inventory' || itemDragState.active) return;
      if (performance.now() < itemDragState.suppressClickUntil) return;
      const slotLocation = getInventoryWorldLocationFromClient(event.clientX, event.clientY);
      if (!slotLocation) return;
    });
    ...
    function buildItemSlotMarkup(location, item, { shortcutLabel = '', selected = false, variant = 'default' } = {}) {
      ...
      const dataset = location.type === 'inventory'
        ...
        : location.type === 'hotbar'
          ? `data-location-type="hotbar" data-slot-index="${location.index}"`
        : location.type === 'chest'
      ...
      if (!item) {
        const emptyLabel = isUpgradeVariant && location.type === 'inventory' ? '' : placeholder.label;
        return `
          <div class="${classes}" ${dataset}>
            <div class="item-slot-inner">
              ...
              <div class="item-slot-copy">
                <strong class="item-slot-empty-label">${emptyLabel}</strong>
                <span class="item-slot-empty-hint">${isUpgradeVariant ? '' : placeholder.hint}</span>
              </div>
            </div>
          </div>
        `;
      ...
      return `
        <div class="${classes}" ${dataset}>
          <div class="item-slot-inner">
            <div class="item-slot-topline">
              <span class="item-slot-shortcut">${shortcutLabel}</span>
              <span class="item-slot-level ${item.kind === 'ore' ? 'item-stack-count' : ''}">${item.kind === 'ore' ? `x${item.amount}` : formatItemLevelLabel(item.level)}</span>
            </div>
            ...
            <div class="item-slot-copy">
              ...
              <span class="item-slot-meta">${isUpgradeVariant ? '' : getItemDisplayMeta(item)}</span>
            </div>
          </div>
        </div>
      `;
    ...
    function applyMarkupToExistingSlotElement(element, location, item, { shortcutLabel = '', selected = false, variant = 'default' } = {}) {
      if (!element) return;
      const key = getLocationKey(location);
      const isDragSource = itemDragState.active && getLocationKey(itemDragState.source) === key;
      const isDragTarget = itemDragState.active && getLocationKey(itemDragState.target) === key;
      ...
      ` : `
        <div class="item-slot-inner">
          ...
          <div class="item-slot-icon-shell"></div>
          <div class="item-slot-copy">
            <strong class="item-slot-empty-label">${isUpgradeVariant && location.type === 'inventory' ? '' : placeholder.label}</strong>
            <span class="item-slot-empty-hint">${isUpgradeVariant ? '' : placeholder.hint}</span>
          </div>
        </div>
      `;
    ...
    function renderInventoryPanels() {
      ...
      const inventorySlotCapacity = getInventorySlotCapacity();
      const backpackBonus = inventorySlotCapacity - BASE_INVENTORY_SLOT_COUNT;
      const inventoryColumns = getInventoryGridColumnCount(inventorySlotCapacity);
      const upgradeColumns = getUpgradeInventoryColumnCount(inventorySlotCapacity);
      const activeNotice = getActiveInventoryNotice();
      ...
      if (inventoryItemCountText) {
        inventoryItemCountText.textContent = backpackBonus > 0
          ? `${inventorySlotCapacity} slots (${BASE_INVENTORY_SLOT_COUNT} base + ${backpackBonus} da mochila) | arraste para reordenar e para a hotbar`
          : `${BASE_INVENTORY_SLOT_COUNT} slots | arraste para reordenar e para a hotbar`;
      }
      if (inventoryInfoText) {
        const equippedItem = getEquippedHotbarItem();
        inventoryInfoText.textContent = activeNotice || (equippedItem
          ? `${getItemDisplayName(equippedItem)} selecionado no slot ${itemState.equippedSlot + 1}. Arraste itens para a barra inferior para mudar as teclas numericas.`
          : 'Arraste itens entre o inventario e a barra inferior. Os 5 slots da barra correspondem as teclas 1-5. A lanterna vai no slot acima da cabeca e a mochila no slot ao lado direito do personagem.');
      }

      if (inventoryItemGrid) {
        inventoryItemGrid.style.gridTemplateColumns = `repeat(${inventoryColumns}, minmax(0, 1fr))`;
      ...
      if (upgradeInventoryGrid) {
        upgradeInventoryGrid.style.gridTemplateColumns = `repeat(${upgradeColumns}, minmax(0, 1fr))`;
      ...
      applyMarkupToExistingSlotElement(inventoryBackpackSlot, { type: 'equipment', slot: 'backpack' }, inventoryState.equipment.backpack);
      applyMarkupToExistingSlotElement(upgradeHeadSlot, { type: 'equipment', slot: 'head' }, inventoryState.equipment.head, { variant: 'upgrade' });
      applyMarkupToExistingSlotElement(upgradeBackpackSlot, { type: 'equipment', slot: 'backpack' }, inventoryState.equipment.backpack, { variant: 'upgrade' });
      applyMarkupToExistingSlotElement(upgradeWorkbenchSlot, { type: 'workbench' }, workbenchState.item, { variant: 'upgrade' });
    ...
    function updateChestRecoveryPanel() {
      if (!chestRecoveryPanel || !chestRecoveryActions || !chestRecoveryHint) return;

      const atHome = areaState.currentArea === 'home';
      chestRecoveryPanel.hidden = !atHome;
      chestRecoveryPanel.setAttribute('aria-hidden', String(!atHome));
      if (!atHome) {
        chestRecoveryActions.innerHTML = '';
        return;
      }

      const availableKinds = HOME_CHEST_RECOVERY_ITEM_KINDS.filter((kind) => !hasItemKindAnywhere(kind));
      chestRecoveryHint.textContent = availableKinds.length
        ? 'Cada botao libera 1 unidade Level 1 apenas se esse item nao existir no bau, no inventario, nos equipamentos, na bancada ou no chao.'
        : 'Nenhuma opcao gratis esta liberada agora. O item precisa estar ausente em todo o jogo para aparecer como disponivel.';

      chestRecoveryActions.innerHTML = HOME_CHEST_RECOVERY_ITEM_KINDS.map((kind) => {
        const previewItem = { kind, level: 1 };
        const available = !hasItemKindAnywhere(kind);
        const statusText = available
          ? 'Pegar 1 unidade gratis | Lvl 1'
          : 'Indisponivel: voce ja possui esse item';
        return `
          <button
            class="chest-recovery-button"
            type="button"
            data-recovery-kind="${kind}"
            data-available="${available ? 'true' : 'false'}"
            ${available ? '' : 'disabled'}
          >
            <span class="chest-recovery-button-icon" aria-hidden="true">
              ${createItemIconMarkup(previewItem)}
            </span>
            <span class="chest-recovery-button-copy">
              <strong>${getItemDisplayName(previewItem)}</strong>
    ...
    function updateChestGrid() {
      if (chestGrid) {
        chestGrid.style.gridTemplateColumns = `repeat(${getInventoryGridColumnCount(getInventorySlotCapacity())}, minmax(0, 1fr))`;
    ...
    function updateUpgradePanel() {
      ...
      if (!item) {
        if (upgradeSelectedKind) upgradeSelectedKind.textContent = 'Mesa vazia';
        if (upgradeSelectedName) upgradeSelectedName.textContent = 'Arraste um item para a bancada';
        if (upgradeSelectedStats) upgradeSelectedStats.textContent = 'Picareta, espada, crossbow, lanterna e mochila podem subir de nivel aqui.';
        ...
        if (upgradeSelectedHint) {
          upgradeSelectedHint.textContent = activeNotice || 'Use itens do inventario, da cabeca ou da mochila. Os custos podem usar recursos do inventario e do bau.';
        }
      } else {
        ...
        const nextInfo = nextUpgrade
          ? (item.kind === 'flashlight'
            ? `Proximo ${formatItemLevelLabel(nextUpgrade.level)} | Alc ${nextUpgrade.distance.toFixed(0)} | Feixe ${(nextUpgrade.angle * 180 / Math.PI).toFixed(0)} deg`
            : item.kind === 'backpack'
              ? `Proximo ${formatItemLevelLabel(nextUpgrade.level)} | +${nextUpgrade.extraSlots} slots`
            : `Proximo ${formatItemLevelLabel(nextUpgrade.level)} | Dano ${nextUpgrade.damage}${nextUpgrade.range ? ` | Alc ${nextUpgrade.range}` : ''}`)
          : 'Nivel maximo atingido';
        if (upgradeSelectedKind) upgradeSelectedKind.textContent = `${itemKindLabels[item.kind]} | ${formatItemLevelLabel(item.level)}`;
        if (upgradeSelectedName) upgradeSelectedName.textContent = getItemDisplayName(item);
        ...
        if (upgradeSelectedItemButton) {
          if (!nextUpgrade) {
            upgradeSelectedItemButton.disabled = true;
            upgradeSelectedItemButton.textContent = 'Nivel maximo';
          } else {
            upgradeSelectedItemButton.disabled = !canSpendCost(nextUpgrade.cost);
            upgradeSelectedItemButton.textContent = `Melhorar para ${formatItemLevelLabel(nextUpgrade.level)}`;
          }
        }
        if (upgradeSelectedHint) {
          if (activeNotice) {
            upgradeSelectedHint.textContent = activeNotice;
          } else if (!nextUpgrade) {
            upgradeSelectedHint.textContent = 'Este item ja chegou ao nivel maximo.';
          } else {
            const readiness = canSpendCost(nextUpgrade.cost) ? 'Custo pronto' : 'Custo';
            upgradeSelectedHint.textContent = `${readiness}: ${formatCost(nextUpgrade.cost)} | ${nextInfo}`;
          }
        }
      }
    }
    ...
    function setActivePanel(panelName) {
      ...
      for (const [name, panel] of Object.entries(panelMap)) {
        const visible = name === panelName && !(panelName === 'inventory' && name === 'inventory');
    ...
    function openPanel(panelName) {
      ...
      if (panelName === 'inventory') {
        ensureInventoryWorldBuilt();
        inventoryWorldState.enabled = true;
        applyInventoryPresentationSnapshot();
      } else {
        inventoryWorldState.enabled = false;
      }
    ...
    function closeActivePanel() {
      ...
      if (itemDragState.active) clearItemDragState();
      const closingInventory = uiState.activePanel === 'inventory';
      inventoryWorldState.enabled = false;
      if (closingInventory) {
        restoreInventoryPresentationSnapshot();
      }
      setActivePanel(null);
      itemState.gameActive = true;
      queuePointerRecapture();
    ...
    chestRecoveryActions.addEventListener('click', (event) => {
      const button = event.target.closest('[data-recovery-kind]');
      if (!button || button.disabled) return;
      grantHomeChestRecoveryItem(button.dataset.recoveryKind);
    });
...
function releaseWorldDrop(drop) {
  if (!drop?.mesh) return;
  if (drop.dropType === 'item') {
    world.group.remove(drop.mesh);
    disposeGroup(drop.mesh);
    return;
  }
  releaseDropMesh(drop);
    ...
    function createDroppedHeadLampModel(level = 1) {
      const stats = getFlashlightStats(level);
      const lightScale = THREE.MathUtils.clamp(stats.lampIntensity / 0.42, 1, 2.4);
      const wrapper = new THREE.Group();
      const group = new THREE.Group();
      wrapper.add(group);
      ...
      const lampMat = new THREE.MeshStandardMaterial({
        color: 0xffee88,
        emissive: 0xffee88,
        emissiveIntensity: 1.0 * lightScale,
        roughness: 0.2,
        metalness: 0.06,
      });
      ...
      const base = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.11, 0.12), housingMat);
      group.add(base);

      const band = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.03, 0.03), rimMat);
      band.position.set(0, -0.045, -0.035);
      group.add(band);

      const lensFrame = new THREE.Mesh(new THREE.CylinderGeometry(0.048, 0.054, 0.055, 18), housingMat);
      lensFrame.rotation.x = Math.PI / 2;
      lensFrame.position.set(0, 0, 0.07);
      group.add(lensFrame);

      const lens = new THREE.Mesh(new THREE.SphereGeometry(0.046, 18, 14), lampMat);
      lens.scale.z = 0.7;
      lens.position.set(0, 0, 0.082);
      group.add(lens);

      const halo = new THREE.Mesh(new THREE.PlaneGeometry(0.28, 0.28), haloMat);
      halo.position.set(0, 0, 0.14);
      group.add(halo);

      const glow = new THREE.PointLight(0xffe7a8, 0.55 * lightScale, 1.55 + Math.max(0, level - 1) * 0.14, 2);
      glow.position.set(0, 0, 0.14);
      group.add(glow);

      group.rotation.x = 0.18;
      applyModelShadows(wrapper);
      halo.castShadow = false;
      halo.receiveShadow = false;

      const baseLensIntensity = lampMat.emissiveIntensity;
      const baseGlowIntensity = glow.intensity;
      const baseHaloOpacity = haloMat.opacity;
      wrapper.userData.update = (elapsedTime) => {
        const pulse = 0.78 + (Math.sin(elapsedTime * 4.8) * 0.5 + 0.5) * 0.44;
        lampMat.emissiveIntensity = baseLensIntensity * pulse;
        glow.intensity = baseGlowIntensity * pulse;
        haloMat.opacity = baseHaloOpacity * (0.86 + Math.sin(elapsedTime * 4.8 + 0.6) * 0.16);
      };
      return wrapper;
    }

    function createDroppedInventoryVisual(item) {
      if (!item || item.kind === 'ore') return null;
      switch (item.kind) {
        case 'pickaxe':
          return createGamePickaxe(item.level);
        case 'sword':
          return createGameSword(item.level);
        case 'crossbow':
          return createGameCrossbow(item.level);
        case 'flashlight':
          return createDroppedHeadLampModel(item.level);
        case 'backpack':
          return createLiteralHtmlBackpackModel(item.level);
        default:
          return null;
      }
    }

    function buildDroppedInventoryVisualRoot(item, visual) {
      const root = new THREE.Group();
      const visualRoot = visual ?? createDroppedInventoryVisual(item);
      if (!visualRoot) return null;

      const dropScale = item?.kind === 'backpack' ? 0.88 : item?.kind === 'flashlight' ? 0.94 : 0.9;
      visualRoot.scale.multiplyScalar(dropScale);
      ...
      visualRoot.position.sub(center);
      root.add(visualRoot);

      const updates = [];
      if (typeof visualRoot.userData.update === 'function') {
        updates.push((elapsedTime, delta) => visualRoot.userData.update(elapsedTime, delta));
      }
      if (Array.isArray(visualRoot.userData.auras) && visualRoot.userData.auras.length) {
        updates.push((elapsedTime, delta) => {
          for (const aura of visualRoot.userData.auras) {
            aura.rotation.z += delta * (aura.userData.reverse ? -2.6 : 2.6);
          }
        });
      }
      if (Array.isArray(visualRoot.userData.rings) && visualRoot.userData.rings.length) {
        updates.push((elapsedTime, delta) => {
          for (const ring of visualRoot.userData.rings) {
            ring.rotation.z += delta * (ring.userData.reverse ? -2.2 : 2.2);
          }
        });
      }
      if (updates.length) {
        root.userData.update = (elapsedTime, delta = 0.016) => {
          for (const update of updates) update(elapsedTime, delta);
        };
      }

      root.userData.restOffsetY = Math.max(0.05, size.y * 0.5);
      root.userData.visualRoot = visualRoot;
      return root;
    }

    function getInventoryDropStyle(item) {
      if (!item) {
        return { base: 0x5b6470, accent: 0xa7b9d4, glow: 0x2d3d52, radius: 0.16 };
      }
      if (item.kind === 'ore') {
        const definition = oreDefinitions.find((ore) => ore.id === item.oreId);
        return {
          base: definition?.color ?? 0x7f96ad,
          accent: definition?.emissive ?? 0xa9d4ff,
          glow: definition?.emissive ?? 0x35506b,
          radius: 0.17,
        };
      }
      if (item.kind === 'backpack') {
        const definition = getBackpackDefinition(item.level);
        return {
          base: definition.primary,
          accent: definition.badge,
          glow: definition.glow,
          radius: 0.19 + Math.min(0.04, (definition.level - 1) * 0.01),
        };
      }
      switch (item.kind) {
        case 'pickaxe':
          return { base: 0x69442b, accent: 0xdce7f7, glow: 0x5a89bc, radius: 0.18 };
        case 'sword':
          return { base: 0x4a5368, accent: 0xe4efff, glow: 0x6990d4, radius: 0.18 };
        case 'crossbow':
          return { base: 0x6a4a28, accent: 0xe1c57e, glow: 0x5f83b2, radius: 0.185 };
        case 'flashlight':
          return { base: 0x434b58, accent: 0xffefad, glow: 0x8ab4ff, radius: 0.17 };
        default:
          return { base: 0x5b6470, accent: 0xa7b9d4, glow: 0x2d3d52, radius: 0.16 };
      }
    }

    function createDroppedInventoryMesh(item) {
      const droppedVisual = buildDroppedInventoryVisualRoot(item);
      if (droppedVisual) {
        return droppedVisual;
      }

      const style = getInventoryDropStyle(item);
      const radius = style.radius;
      const group = new THREE.Group();
      const body = new THREE.Mesh(
        new THREE.CylinderGeometry(radius * 0.78, radius * 0.88, radius * 0.5, 8, 1),
        new THREE.MeshStandardMaterial({
          color: style.base,
          emissive: style.glow,
          emissiveIntensity: 0.18,
      ...
      const cap = new THREE.Mesh(
        new THREE.CylinderGeometry(radius * 0.54, radius * 0.62, radius * 0.18, 8, 1),
        new THREE.MeshStandardMaterial({
          color: style.accent,
          emissive: style.glow,
      ...
      if (iconTexture) {
        ...
        sprite.scale.set(radius * 1.62, radius * 1.62, 1);
        sprite.position.set(0, radius * 0.58, 0);
        group.add(sprite);
      }

      group.userData.restOffsetY = radius * 0.27;
      return group;
    }

    function getSafeDropFloorHeight(x, z, fallbackY = 0) {
      return world.floorNoise?.noise ? floorHeightAt(x, z) : fallbackY;
    }

    function spawnDroppedInventoryItem(item, origin, {
      scatterRadius = 0.22,
      pickupDelay = INVENTORY_DROP_PICKUP_DELAY,
    } = {}) {
      const entry = cloneInventoryEntry(item);
      if (!entry || !origin) return null;
      const mesh = createDroppedInventoryMesh(entry);
      const angle = Math.random() * Math.PI * 2;
      ...
      const fallbackFloor = origin.y - world.playerHeight * 0.72;
      const baseFloor = getSafeDropFloorHeight(px, pz, fallbackFloor);
      const restOffsetY = mesh.userData.restOffsetY ?? 0.06;
      mesh.position.set(px, Math.max(origin.y, baseFloor + restOffsetY + 0.02), pz);
      mesh.rotation.set(
        Math.random() * Math.PI,
        Math.random() * Math.PI,
        Math.random() * Math.PI
      ...
      const drop = {
        ...
        mesh,
        velocity: new THREE.Vector3(
          Math.cos(angle) * (0.55 + Math.random() * 0.8),
          1.3 + Math.random() * 0.8,
          Math.sin(angle) * (0.55 + Math.random() * 0.8)
        ),
        ...
        restOffsetY,
        settled: false,
        pickupReadyAt: clock.elapsedTime + pickupDelay,
        pickupUnlocked: false,
        pickupUnlockDistanceSq: INVENTORY_DROP_UNLOCK_RADIUS * INVENTORY_DROP_UNLOCK_RADIUS,
      };
      world.dropItems.push(drop);
      return drop;
    }

    function spawnDroppedInventoryItems(items, origin, {
      scatterRadius = 0.32,
      pickupDelay = INVENTORY_DROP_PICKUP_DELAY,
    } = {}) {
      const drops = [];
      if (!Array.isArray(items) || !origin) return drops;
      items.forEach((item, index) => {
        const drop = spawnDroppedInventoryItem(item, origin, {
          scatterRadius: scatterRadius + Math.min(0.28, index * 0.03),
          pickupDelay,
        });
    ...
    function collectNearbyDrops() {
      ...
      for (const drop of world.dropItems) {
        ...
        if (drop.dropType === 'item') {
          if (!drop.pickupUnlocked) {
            if (distanceSq > drop.pickupUnlockDistanceSq) drop.pickupUnlocked = true;
          } else if (clock.elapsedTime >= drop.pickupReadyAt && distanceSq <= INVENTORY_DROP_PICKUP_RADIUS * INVENTORY_DROP_PICKUP_RADIUS) {
            const originalItem = cloneInventoryEntry(drop.item);
            const leftoverItem = addInventoryItemToInventorySlots(originalItem);
            const collectedAmount = getInventoryEntryUnitCount(originalItem) - getInventoryEntryUnitCount(leftoverItem);
            if (collectedAmount > 0) {
              inventoryChanged = true;
              if (originalItem.kind === 'ore') {
                collectedTotals.set(originalItem.oreId, (collectedTotals.get(originalItem.oreId) ?? 0) + collectedAmount);
              } else {
                collectedItems.push(originalItem);
              }
            }
            if (!leftoverItem) {
              releaseWorldDrop(drop);
              continue;
            }
            drop.item = leftoverItem;
          }
        } else if (distanceSq <= INVENTORY_DROP_PICKUP_RADIUS * INVENTORY_DROP_PICKUP_RADIUS) {
      ...
      if (inventoryChanged) {
        ...
        for (const item of collectedItems) {
          ...
          uiWorldPosition.x += randomRange(-0.1, 0.1);
          uiWorldPosition.z += randomRange(-0.1, 0.1);
          const levelSuffix = Number.isFinite(item.level) ? ` ${formatItemLevelLabel(item.level)}` : '';
          spawnFloatingUiText(uiWorldPosition, `+ ${getItemDisplayName(item)}${levelSuffix}`, 'pickup-note', 1.2);
          noticeIndex++;
        }
        refreshResourceUi();
        saveGame();
    ...
    function updateMiningHud() {
      ...
      if (!isItemKindEquipped('pickaxe')) {
        ...
        miningChip.textContent = pickaxeHotbarSlot !== -1
          ...
          : 'Mineracao: mova a picareta para a barra inferior';
        return;
      }
    ...
    function updateOreAndDrops(delta) {
      ...
      for (const drop of world.dropItems) {
        if (drop.dropType === 'item') {
          const floorY = getSafeDropFloorHeight(
            drop.mesh.position.x,
            drop.mesh.position.z,
            drop.mesh.position.y - drop.restOffsetY
          ) + drop.restOffsetY;

          if (!drop.settled) {
            drop.velocity.y -= INVENTORY_DROP_GRAVITY * delta;
            drop.mesh.position.addScaledVector(drop.velocity, delta);
            drop.mesh.rotation.x += drop.angularVelocity.x * delta;
            drop.mesh.rotation.y += drop.angularVelocity.y * delta;
            drop.mesh.rotation.z += drop.angularVelocity.z * delta;
            drop.velocity.x = damp(drop.velocity.x, 0, drop.mesh.position.y <= floorY + 0.05 ? 8.5 : 1.9, delta);
            drop.velocity.z = damp(drop.velocity.z, 0, drop.mesh.position.y <= floorY + 0.05 ? 8.5 : 1.9, delta);
            drop.angularVelocity.x = damp(drop.angularVelocity.x, 0, 6.2, delta);
            drop.angularVelocity.y = damp(drop.angularVelocity.y, 0, 5.4, delta);
            drop.angularVelocity.z = damp(drop.angularVelocity.z, 0, 6.2, delta);

            if (drop.mesh.position.y <= floorY) {
              drop.mesh.position.y = floorY;
              if (Math.abs(drop.velocity.y) > 0.55) {
                drop.velocity.y = -drop.velocity.y * 0.16;
                drop.velocity.x *= 0.72;
                drop.velocity.z *= 0.72;
                drop.angularVelocity.multiplyScalar(0.76);
              } else {
                drop.velocity.set(0, 0, 0);
                drop.angularVelocity.set(0, 0, 0);
                drop.settled = true;
              }
            }
          } else {
          ...
          drop.mesh.userData.update?.(clock.elapsedTime, delta, drop);
          continue;
        }
    ...
    function animate() {
      ...
      const flashlightStats = getFlashlightStats();
      const flashlightEnabled = hasHeadFlashlightEquipped() && itemState.flashlightOn && !inventoryWorldState.enabled;
      flashlightModel.visible = false;
      ...
      updateInventoryWorldPresentation(delta);
      if (devModeState.enabled) {
